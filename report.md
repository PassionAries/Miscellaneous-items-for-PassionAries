# 源接口健康检测报告

最近更新时间：2026-07-04 02:14 CST

**总源数:** 77 | **检测关键词:** 你好

| 状态 | 资源名称 | 地址 | API | 搜索功能 | 成功次数 | 失败次数 | 成功率 | 最近7天趋势 |
|------|---------|-----|-----|---------|---------:|--------:|-------:|--------------|
| 🚨 | 🎬金鹰点播 | [Link](https://jinyingzy.com) | [Link](https://www.jinyingzy.com/provide/vod?ct=xml) | ❌ | 0 | 26 | 0.0% | ❌❌❌❌❌❌❌ |
| 🚨 | 🎬百度云zy | [Link](https://bdzy1.com) | [Link](https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://api.apibdzy.com/api.php/provide/vod) | ❌ | 16 | 12 | 57.1% | ❌❌❌❌❌❌❌ |
| 🚨 | 🎬艾旦影视 | [Link](https://lovedan.net) | [Link](https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://lovedan.net/api.php/provide/vod) | ❌ | 16 | 12 | 57.1% | ❌❌❌❌❌❌❌ |
| 🚨 | 🔞优优资源 | [Link](https://www.yytv4.cc) | [Link](https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://www.yytv4.cc/api.php/provide/vod) | ❌ | 0 | 28 | 0.0% | ❌❌❌❌❌❌❌ |
| ❌ | 🎬极速资源 | [Link](https://jszyapi.com) | [Link](https://jszyapi.com/api.php/provide/vod) | ❌ | 28 | 2 | 93.3% | ✅✅✅✅✅❌❌ |
| ✅ | 🎬-爱奇艺- | [Link](https://iqiyizyapi.com) | [Link](https://iqiyizyapi.com/api.php/provide/vod) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬豆瓣资源 | [Link](https://dbzy.tv) | [Link](https://caiji.dbzy5.com/api.php/provide/vod) | 无结果 | 25 | 5 | 83.3% | ✅✅❌❌✅✅✅ |
| ✅ | 🎬天涯资源 | [Link](https://tyyszy.com/) | [Link](https://tyyszy.com/api.php/provide/vod) | ❌ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬天涯影视 | [Link](https://tyyszyapi.com/) | [Link](https://tyyszyapi.com/api.php/provide/vod) | ❌ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬茅台资源 | [Link](https://mtzy.me) | [Link](https://caiji.maotaizy.cc/api.php/provide/vod) | 无结果 | 25 | 5 | 83.3% | ✅✅❌❌✅✅✅ |
| ✅ | 🎬卧龙资源 | [Link](https://wolongzyw.com) | [Link](https://wolongzyw.com/api.php/provide/vod) | ❌ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬iKun资源 | [Link](https://ikunzy.com) | [Link](https://ikunzyapi.com/api.php/provide/vod) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬电影天堂 | [Link](http://caiji.dyttzyapi.com) | [Link](http://caiji.dyttzyapi.com/api.php/provide/vod) | ❌ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬猫眼资源 | [Link](https://www.maoyanzy.com) | [Link](https://api.maoyanapi.top/api.php/provide/vod) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬量子资源 | [Link](https://cj.lzcaiji.com) | [Link](https://cj.lzcaiji.com/api.php/provide/vod) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬360 资源 | [Link](https://360zy.com) | [Link](https://360zyzz.com/api.php/provide/vod) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬魔都资源 | [Link](https://www.moduzy.net) | [Link](https://www.mdzyapi.com/api.php/provide/vod) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬非凡资源 | [Link](https://cj.ffzyapi.com) | [Link](https://api.ffzyapi.com/api.php/provide/vod) | ❌ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬暴风资源 | [Link](https://bfzy.tv) | [Link](https://bfzyapi.com/api.php/provide/vod) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬最大资源 | [Link](https://zuida.xyz) | [Link](https://api.zuidapi.com/api.php/provide/vod) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬无尽资源 | [Link](https://wujinzy.com) | [Link](https://api.wujinapi.me/api.php/provide/vod) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬新浪资源 | [Link](https://xinlangapi.com) | [Link](https://api.xinlangapi.com/xinlangapi.php/provide/vod) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬旺旺资源 | [Link](https://api.wwzy.tv) | [Link](https://api.wwzy.tv/api.php/provide/vod) | ❌ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬速播资源 | [Link](https://www.subozy.com) | [Link](https://subocaiji.com/api.php/provide/vod) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬飘零资源 | [Link](https://p2100.net) | [Link](https://p2100.net/api.php/provide/vod) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬U酷影视 | [Link](https://www.ukuzy.com) | [Link](https://api.ukuapi88.com/api.php/provide/vod) | ✅ | 25 | 5 | 83.3% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬光速资源 | [Link](https://api.guangsuapi.com) | [Link](https://api.guangsuapi.com/api.php/provide/vod) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬红牛资源 | [Link](https://www.hongniuzy.com) | [Link](https://www.hongniuzy2.com/api.php/provide/vod) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬魔都动漫 | [Link](https://caiji.moduapi.cc) | [Link](https://caiji.moduapi.cc/api.php/provide/vod) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬如意资源 | [Link](https://www.ryzyw.com) | [Link](https://cj.rycjapi.com/api.php/provide/vod) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬豪华资源 | [Link](https://www.haohuazy.com) | [Link](https://hhzyapi.com/api.php/provide/vod) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬1080资源 | [Link](https://1080zyku.com) | [Link](https://api.1080zyku.com/inc/apijson.php/) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬无水印资源 | [Link](https://wsyzy.net/) | [Link](https://api.wsyzy.net/api.php/provide/vod) | ❌ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬量子影视 | [Link](https://lzizy.net) | [Link](https://cj.lziapi.com/api.php/provide/vod) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬最大点播 | [Link](https://zuidazy.co) | [Link](https://zuidazy.me/api.php/provide/vod) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬无尽影视 | [Link](https://wujinzy.com) | [Link](https://api.wujinapi.com/api.php/provide/vod) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬旺旺短剧 | [Link](https://wwzy.tv) | [Link](https://wwzy.tv/api.php/provide/vod) | ❌ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬优质资源 | [Link](https://1080zyk4.com) | [Link](https://api.yzzy-api.com/inc/apijson.php) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬虎牙资源 | [Link](https://www.huyaapi.com) | [Link](https://www.huyaapi.com/api.php/provide/vod) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬鸭鸭资源 | [Link](https://yayazy3.com) | [Link](https://cj.yayazy.net/api.php/provide/vod) | ❌ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬索尼资源 | [Link](https://suonizy.net) | [Link](https://suoniapi.com/api.php/provide/vod) | ❌ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬快车资源 | [Link](https://kuaichezy.com) | [Link](https://caiji.kuaichezy.org/api.php/provide/vod) | ❌ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬闪电资源 | [Link](https://shandianzy.com) | [Link](https://xsd.sdzyapi.com/api.php/provide/vod) | ❌ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬樱花资源 | [Link](https://yhzy.cc) | [Link](https://m3u8.apiyhzy.com/api.php/provide/vod) | ❌ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🔞麻豆视频 | [Link](https://91md.me) | [Link](https://91md.me/api.php/provide/vod) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🔞--AIvin- | [Link](http://lbapiby.com) | [Link](http://lbapiby.com/api.php/provide/vod) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🔞155-资源 | [Link](https://155zy2.com) | [Link](https://155api.com/api.php/provide/vod) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🔞玉兔资源 | [Link](https://apiyutu.com) | [Link](https://apiyutu.com/api.php/provide/vod) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🔞番号资源 | [Link](http://fhapi9.com) | [Link](http://fhapi9.com/api.php/provide/vod) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🔞-老色逼- | [Link](https://apilsbzy1.com) | [Link](https://apilsbzy1.com/api.php/provide/vod) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🔞小鸡资源 | [Link](https://xiaojizy.live) | [Link](https://api.xiaojizy.live/provide/vod) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🔞黄色仓库 | [Link](https://hsckzy.xyz) | [Link](https://hsckzy.xyz/api.php/provide/vod) | ✅ | 23 | 7 | 76.7% | ✅❌✅✅❌✅✅ |
| ✅ | 🔞-大奶子- | [Link](https://apidanaizi.com) | [Link](https://apidanaizi.com/api.php/provide/vod) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🔞jkun资源 | [Link](https://jkunzyapi.com) | [Link](https://jkunzyapi.com/api.php/provide/vod) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🔞乐播资源 | [Link](https://lbapi9.com) | [Link](https://lbapi9.com/api.php/provide/vod) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🔞奶香资源 | [Link](https://Naixxzy.com) | [Link](https://Naixxzy.com/api.php/provide/vod) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🔞森林资源 | [Link](https://slapibf.com) | [Link](https://beiyong.slapibf.com/api.php/provide/vod) | ❌ | 29 | 1 | 96.7% | ✅✅✅✅✅✅✅ |
| ✅ | 🔞辣椒资源 | [Link](https://apilj.com) | [Link](https://apilj.com/api.php/provide/vod) | ✅ | 29 | 1 | 96.7% | ✅✅✅✅✅✅✅ |
| ✅ | 🔞鲨鱼资源 | [Link](https://shayuapi.com) | [Link](https://shayuapi.com/api.php/provide/vod) | ✅ | 28 | 2 | 93.3% | ✅✅✅✅✅✅✅ |
| ✅ | 🔞豆豆资源 | [Link](https://doudouzy.com) | [Link](https://api.douapi.cc/api.php/provide/vod) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🔞滴滴资源 | [Link](https://didizy.com) | [Link](https://api.ddapi.cc/api.php/provide/vod) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🔞黑料资源 | [Link](https://heiliaozy.cc) | [Link](https://www.heiliaozyapi.com/api.php/provide/vod) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🔞百万资源 | [Link](https://api.bwzym3u8.com) | [Link](https://api.bwzyz.com/api.php/provide/vod) | ❌ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🔞桃花资源 | [Link](https://thzy8.me) | [Link](https://thzy1.me/api.php/provide/vod) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🔞精品资源 | [Link](https://www.jingpinx.com) | [Link](https://www.jingpinx.com/api.php/provide/vod) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🔞 CK-资源 | [Link](https://ckzy.me) | [Link](https://ckzy.me/api.php/provide/vod) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🔞souavZY | [Link](https://api.souavzyw.net) | [Link](https://api.souavzyw.net/api.php/provide/vod) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🔞细胞资源 | [Link](https://www.xxibaozyw.com) | [Link](https://www.xxibaozyw.com/api.php/provide/vod) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🔞香蕉资源 | [Link](https://www.xiangjiaozyw.com) | [Link](https://www.xiangjiaozyw.com/api.php/provide/vod) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🔞-美少女- | [Link](https://www.msnii.com) | [Link](https://www.msnii.com/api/json.php) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🔞-黄AVZY | [Link](https://www.pgxdy.com) | [Link](https://www.pgxdy.com/api/json.php) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🔞白嫖资源 | [Link](https://www.kxgav.com) | [Link](https://www.kxgav.com/api/json.php) | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🔞杏吧资源 | [Link](https://xingba111.com) | [Link](https://xingba222.com/api.php/provide/vod) | 无结果 | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🔞大地资源 | [Link](https://dadizy11.com) | [Link](https://dadiapi.com/feifei) | 不匹配 | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🔞色猫资源 | [Link](https://semaozy1.com) | [Link](https://caiji.semaozy.net/inc/apijson_vod.php/provide/vod) | 无结果 | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🔞-奥斯卡- | [Link](https://aosikazy.com) | [Link](https://aosikazy.com/api.php/provide/vod) | ❌ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🔞丝袜资源 | [Link](https://siwazyw.tv) | [Link](https://siwazyw.tv/api.php/provide/vod) | ❌ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |

<details>
<summary>📜 点击展开查看历史检测数据 (JSON)</summary>

```json
[
  {
    "date": "2026-06-09",
    "keyword": "你好",
    "results": [
      {
        "name": "🎬-爱奇艺-",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豆瓣资源",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬天涯资源",
        "api": "https://tyyszy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬天涯影视",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬茅台资源",
        "api": "https://caiji.maotaizy.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬卧龙资源",
        "api": "https://wolongzyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬iKun资源",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬电影天堂",
        "api": "http://caiji.dyttzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子资源",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬360 资源",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬极速资源",
        "api": "https://jszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都资源",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬非凡资源",
        "api": "https://api.ffzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬暴风资源",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大资源",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽资源",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪资源",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬旺旺资源",
        "api": "https://api.wwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬速播资源",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰点播",
        "api": "https://jinyingzy.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬飘零资源",
        "api": "https://p2100.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷影视",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬光速资源",
        "api": "https://api.guangsuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛资源",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都动漫",
        "api": "https://caiji.moduapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬如意资源",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华资源",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云zy",
        "api": "https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬1080资源",
        "api": "https://api.1080zyku.com/inc/apijson.php/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无水印资源",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬艾旦影视",
        "api": "https://lovedan.net/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬量子影视",
        "api": "https://cj.lziapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大点播",
        "api": "https://zuidazy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽影视",
        "api": "https://api.wujinapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬旺旺短剧",
        "api": "https://wwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬优质资源",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬虎牙资源",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬鸭鸭资源",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬索尼资源",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬快车资源",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬闪电资源",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬樱花资源",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞麻豆视频",
        "api": "https://91md.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞--AIvin-",
        "api": "http://lbapiby.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞155-资源",
        "api": "https://155api.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞玉兔资源",
        "api": "https://apiyutu.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞番号资源",
        "api": "http://fhapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-老色逼-",
        "api": "https://apilsbzy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞优优资源",
        "api": "https://www.yytv4.cc/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞小鸡资源",
        "api": "https://api.xiaojizy.live/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黄色仓库",
        "api": "https://hsckzy.xyz/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-大奶子-",
        "api": "https://apidanaizi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞jkun资源",
        "api": "https://jkunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞乐播资源",
        "api": "https://lbapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奶香资源",
        "api": "https://Naixxzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞森林资源",
        "api": "https://beiyong.slapibf.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞辣椒资源",
        "api": "https://apilj.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞鲨鱼资源",
        "api": "https://shayuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞豆豆资源",
        "api": "https://api.douapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞滴滴资源",
        "api": "https://api.ddapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黑料资源",
        "api": "https://www.heiliaozyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞百万资源",
        "api": "https://api.bwzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞桃花资源",
        "api": "https://thzy1.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞精品资源",
        "api": "https://www.jingpinx.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞 CK-资源",
        "api": "https://ckzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞souavZY",
        "api": "https://api.souavzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞细胞资源",
        "api": "https://www.xxibaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞香蕉资源",
        "api": "https://www.xiangjiaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-美少女-",
        "api": "https://www.msnii.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-黄AVZY",
        "api": "https://www.pgxdy.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞白嫖资源",
        "api": "https://www.kxgav.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞杏吧资源",
        "api": "https://xingba222.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞大地资源",
        "api": "https://dadiapi.com/feifei",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🔞色猫资源",
        "api": "https://caiji.semaozy.net/inc/apijson_vod.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞-奥斯卡-",
        "api": "https://aosikazy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞丝袜资源",
        "api": "https://siwazyw.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      }
    ]
  },
  {
    "date": "2026-06-09",
    "keyword": "你好",
    "results": [
      {
        "name": "🎬-爱奇艺-",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豆瓣资源",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬天涯资源",
        "api": "https://tyyszy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬天涯影视",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬茅台资源",
        "api": "https://caiji.maotaizy.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬卧龙资源",
        "api": "https://wolongzyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬iKun资源",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬电影天堂",
        "api": "http://caiji.dyttzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子资源",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬360 资源",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬极速资源",
        "api": "https://jszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都资源",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬非凡资源",
        "api": "https://api.ffzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬暴风资源",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大资源",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽资源",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪资源",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬旺旺资源",
        "api": "https://api.wwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬速播资源",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰点播",
        "api": "https://jinyingzy.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬飘零资源",
        "api": "https://p2100.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷影视",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬光速资源",
        "api": "https://api.guangsuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛资源",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都动漫",
        "api": "https://caiji.moduapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬如意资源",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华资源",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云zy",
        "api": "https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬1080资源",
        "api": "https://api.1080zyku.com/inc/apijson.php/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无水印资源",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬艾旦影视",
        "api": "https://lovedan.net/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬量子影视",
        "api": "https://cj.lziapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大点播",
        "api": "https://zuidazy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽影视",
        "api": "https://api.wujinapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬旺旺短剧",
        "api": "https://wwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬优质资源",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬虎牙资源",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬鸭鸭资源",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬索尼资源",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬快车资源",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬闪电资源",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬樱花资源",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞麻豆视频",
        "api": "https://91md.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞--AIvin-",
        "api": "http://lbapiby.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞155-资源",
        "api": "https://155api.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞玉兔资源",
        "api": "https://apiyutu.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞番号资源",
        "api": "http://fhapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-老色逼-",
        "api": "https://apilsbzy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞优优资源",
        "api": "https://www.yytv4.cc/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞小鸡资源",
        "api": "https://api.xiaojizy.live/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黄色仓库",
        "api": "https://hsckzy.xyz/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-大奶子-",
        "api": "https://apidanaizi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞jkun资源",
        "api": "https://jkunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞乐播资源",
        "api": "https://lbapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奶香资源",
        "api": "https://Naixxzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞森林资源",
        "api": "https://beiyong.slapibf.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞辣椒资源",
        "api": "https://apilj.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞鲨鱼资源",
        "api": "https://shayuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞豆豆资源",
        "api": "https://api.douapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞滴滴资源",
        "api": "https://api.ddapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黑料资源",
        "api": "https://www.heiliaozyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞百万资源",
        "api": "https://api.bwzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞桃花资源",
        "api": "https://thzy1.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞精品资源",
        "api": "https://www.jingpinx.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞 CK-资源",
        "api": "https://ckzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞souavZY",
        "api": "https://api.souavzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞细胞资源",
        "api": "https://www.xxibaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞香蕉资源",
        "api": "https://www.xiangjiaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-美少女-",
        "api": "https://www.msnii.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-黄AVZY",
        "api": "https://www.pgxdy.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞白嫖资源",
        "api": "https://www.kxgav.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞杏吧资源",
        "api": "https://xingba222.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞大地资源",
        "api": "https://dadiapi.com/feifei",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🔞色猫资源",
        "api": "https://caiji.semaozy.net/inc/apijson_vod.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞-奥斯卡-",
        "api": "https://aosikazy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞丝袜资源",
        "api": "https://siwazyw.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      }
    ]
  },
  {
    "date": "2026-06-09",
    "keyword": "你好",
    "results": [
      {
        "name": "🎬-爱奇艺-",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豆瓣资源",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬天涯资源",
        "api": "https://tyyszy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬天涯影视",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬茅台资源",
        "api": "https://caiji.maotaizy.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬卧龙资源",
        "api": "https://wolongzyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬iKun资源",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬电影天堂",
        "api": "http://caiji.dyttzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子资源",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬360 资源",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬极速资源",
        "api": "https://jszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都资源",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬非凡资源",
        "api": "https://api.ffzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬暴风资源",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大资源",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽资源",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪资源",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬旺旺资源",
        "api": "https://api.wwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬速播资源",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰点播",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://jinyingzy.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬飘零资源",
        "api": "https://p2100.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷影视",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬光速资源",
        "api": "https://api.guangsuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛资源",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都动漫",
        "api": "https://caiji.moduapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬如意资源",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华资源",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云zy",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬1080资源",
        "api": "https://api.1080zyku.com/inc/apijson.php/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无水印资源",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬艾旦影视",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://lovedan.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子影视",
        "api": "https://cj.lziapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大点播",
        "api": "https://zuidazy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽影视",
        "api": "https://api.wujinapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬旺旺短剧",
        "api": "https://wwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬优质资源",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬虎牙资源",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬鸭鸭资源",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬索尼资源",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬快车资源",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬闪电资源",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬樱花资源",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞麻豆视频",
        "api": "https://91md.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞--AIvin-",
        "api": "http://lbapiby.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞155-资源",
        "api": "https://155api.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞玉兔资源",
        "api": "https://apiyutu.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞番号资源",
        "api": "http://fhapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-老色逼-",
        "api": "https://apilsbzy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞优优资源",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://www.yytv4.cc/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞小鸡资源",
        "api": "https://api.xiaojizy.live/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黄色仓库",
        "api": "https://hsckzy.xyz/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-大奶子-",
        "api": "https://apidanaizi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞jkun资源",
        "api": "https://jkunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞乐播资源",
        "api": "https://lbapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奶香资源",
        "api": "https://Naixxzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞森林资源",
        "api": "https://beiyong.slapibf.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞辣椒资源",
        "api": "https://apilj.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞鲨鱼资源",
        "api": "https://shayuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞豆豆资源",
        "api": "https://api.douapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞滴滴资源",
        "api": "https://api.ddapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黑料资源",
        "api": "https://www.heiliaozyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞百万资源",
        "api": "https://api.bwzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞桃花资源",
        "api": "https://thzy1.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞精品资源",
        "api": "https://www.jingpinx.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞 CK-资源",
        "api": "https://ckzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞souavZY",
        "api": "https://api.souavzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞细胞资源",
        "api": "https://www.xxibaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞香蕉资源",
        "api": "https://www.xiangjiaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-美少女-",
        "api": "https://www.msnii.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-黄AVZY",
        "api": "https://www.pgxdy.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞白嫖资源",
        "api": "https://www.kxgav.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞杏吧资源",
        "api": "https://xingba222.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞大地资源",
        "api": "https://dadiapi.com/feifei",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🔞色猫资源",
        "api": "https://caiji.semaozy.net/inc/apijson_vod.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞-奥斯卡-",
        "api": "https://aosikazy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞丝袜资源",
        "api": "https://siwazyw.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      }
    ]
  },
  {
    "date": "2026-06-09",
    "keyword": "你好",
    "results": [
      {
        "name": "🎬-爱奇艺-",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豆瓣资源",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬天涯资源",
        "api": "https://tyyszy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬天涯影视",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬茅台资源",
        "api": "https://caiji.maotaizy.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬卧龙资源",
        "api": "https://wolongzyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬iKun资源",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬电影天堂",
        "api": "http://caiji.dyttzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子资源",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬360 资源",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬极速资源",
        "api": "https://jszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都资源",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬非凡资源",
        "api": "https://api.ffzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬暴风资源",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大资源",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽资源",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪资源",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬旺旺资源",
        "api": "https://api.wwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬速播资源",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰点播",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://jinyingzy.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬飘零资源",
        "api": "https://p2100.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷影视",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬光速资源",
        "api": "https://api.guangsuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛资源",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都动漫",
        "api": "https://caiji.moduapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬如意资源",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华资源",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云zy",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬1080资源",
        "api": "https://api.1080zyku.com/inc/apijson.php/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无水印资源",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬艾旦影视",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://lovedan.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子影视",
        "api": "https://cj.lziapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大点播",
        "api": "https://zuidazy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽影视",
        "api": "https://api.wujinapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬旺旺短剧",
        "api": "https://wwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬优质资源",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬虎牙资源",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬鸭鸭资源",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬索尼资源",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬快车资源",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬闪电资源",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬樱花资源",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞麻豆视频",
        "api": "https://91md.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞--AIvin-",
        "api": "http://lbapiby.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞155-资源",
        "api": "https://155api.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞玉兔资源",
        "api": "https://apiyutu.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞番号资源",
        "api": "http://fhapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-老色逼-",
        "api": "https://apilsbzy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞优优资源",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://www.yytv4.cc/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞小鸡资源",
        "api": "https://api.xiaojizy.live/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黄色仓库",
        "api": "https://hsckzy.xyz/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-大奶子-",
        "api": "https://apidanaizi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞jkun资源",
        "api": "https://jkunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞乐播资源",
        "api": "https://lbapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奶香资源",
        "api": "https://Naixxzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞森林资源",
        "api": "https://beiyong.slapibf.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞辣椒资源",
        "api": "https://apilj.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞鲨鱼资源",
        "api": "https://shayuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞豆豆资源",
        "api": "https://api.douapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞滴滴资源",
        "api": "https://api.ddapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黑料资源",
        "api": "https://www.heiliaozyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞百万资源",
        "api": "https://api.bwzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞桃花资源",
        "api": "https://thzy1.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞精品资源",
        "api": "https://www.jingpinx.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞 CK-资源",
        "api": "https://ckzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞souavZY",
        "api": "https://api.souavzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞细胞资源",
        "api": "https://www.xxibaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞香蕉资源",
        "api": "https://www.xiangjiaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-美少女-",
        "api": "https://www.msnii.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-黄AVZY",
        "api": "https://www.pgxdy.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞白嫖资源",
        "api": "https://www.kxgav.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞杏吧资源",
        "api": "https://xingba222.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞大地资源",
        "api": "https://dadiapi.com/feifei",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🔞色猫资源",
        "api": "https://caiji.semaozy.net/inc/apijson_vod.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞-奥斯卡-",
        "api": "https://aosikazy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞丝袜资源",
        "api": "https://siwazyw.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      }
    ]
  },
  {
    "date": "2026-06-09",
    "keyword": "你好",
    "results": [
      {
        "name": "🎬-爱奇艺-",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豆瓣资源",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬天涯资源",
        "api": "https://tyyszy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬天涯影视",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬茅台资源",
        "api": "https://caiji.maotaizy.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬卧龙资源",
        "api": "https://wolongzyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬iKun资源",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬电影天堂",
        "api": "http://caiji.dyttzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子资源",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬360 资源",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬极速资源",
        "api": "https://jszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都资源",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬非凡资源",
        "api": "https://api.ffzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬暴风资源",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大资源",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽资源",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪资源",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬旺旺资源",
        "api": "https://api.wwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬速播资源",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰点播",
        "api": "https://www.jinyingzy.com/provide/vod?ct=xml",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬飘零资源",
        "api": "https://p2100.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷影视",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬光速资源",
        "api": "https://api.guangsuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛资源",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都动漫",
        "api": "https://caiji.moduapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬如意资源",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华资源",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云zy",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬1080资源",
        "api": "https://api.1080zyku.com/inc/apijson.php/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无水印资源",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬艾旦影视",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://lovedan.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子影视",
        "api": "https://cj.lziapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大点播",
        "api": "https://zuidazy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽影视",
        "api": "https://api.wujinapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬旺旺短剧",
        "api": "https://wwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬优质资源",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬虎牙资源",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬鸭鸭资源",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬索尼资源",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬快车资源",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬闪电资源",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬樱花资源",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞麻豆视频",
        "api": "https://91md.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞--AIvin-",
        "api": "http://lbapiby.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞155-资源",
        "api": "https://155api.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞玉兔资源",
        "api": "https://apiyutu.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞番号资源",
        "api": "http://fhapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-老色逼-",
        "api": "https://apilsbzy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞优优资源",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://www.yytv4.cc/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞小鸡资源",
        "api": "https://api.xiaojizy.live/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黄色仓库",
        "api": "https://hsckzy.xyz/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞-大奶子-",
        "api": "https://apidanaizi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞jkun资源",
        "api": "https://jkunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞乐播资源",
        "api": "https://lbapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奶香资源",
        "api": "https://Naixxzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞森林资源",
        "api": "https://beiyong.slapibf.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞辣椒资源",
        "api": "https://apilj.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞鲨鱼资源",
        "api": "https://shayuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞豆豆资源",
        "api": "https://api.douapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞滴滴资源",
        "api": "https://api.ddapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黑料资源",
        "api": "https://www.heiliaozyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞百万资源",
        "api": "https://api.bwzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞桃花资源",
        "api": "https://thzy1.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞精品资源",
        "api": "https://www.jingpinx.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞 CK-资源",
        "api": "https://ckzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞souavZY",
        "api": "https://api.souavzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞细胞资源",
        "api": "https://www.xxibaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞香蕉资源",
        "api": "https://www.xiangjiaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-美少女-",
        "api": "https://www.msnii.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-黄AVZY",
        "api": "https://www.pgxdy.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞白嫖资源",
        "api": "https://www.kxgav.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞杏吧资源",
        "api": "https://xingba222.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞大地资源",
        "api": "https://dadiapi.com/feifei",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🔞色猫资源",
        "api": "https://caiji.semaozy.net/inc/apijson_vod.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞-奥斯卡-",
        "api": "https://aosikazy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞丝袜资源",
        "api": "https://siwazyw.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      }
    ]
  },
  {
    "date": "2026-06-09",
    "keyword": "你好",
    "results": [
      {
        "name": "🎬-爱奇艺-",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豆瓣资源",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬天涯资源",
        "api": "https://tyyszy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬天涯影视",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬茅台资源",
        "api": "https://caiji.maotaizy.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬卧龙资源",
        "api": "https://wolongzyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬iKun资源",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬电影天堂",
        "api": "http://caiji.dyttzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子资源",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬360 资源",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬极速资源",
        "api": "https://jszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都资源",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬非凡资源",
        "api": "https://api.ffzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬暴风资源",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大资源",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽资源",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪资源",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬旺旺资源",
        "api": "https://api.wwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬速播资源",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰点播",
        "api": "https://www.jinyingzy.com/provide/vod?ct=xml",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬飘零资源",
        "api": "https://p2100.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷影视",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬光速资源",
        "api": "https://api.guangsuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛资源",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都动漫",
        "api": "https://caiji.moduapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬如意资源",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华资源",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云zy",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬1080资源",
        "api": "https://api.1080zyku.com/inc/apijson.php/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无水印资源",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬艾旦影视",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://lovedan.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子影视",
        "api": "https://cj.lziapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大点播",
        "api": "https://zuidazy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽影视",
        "api": "https://api.wujinapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬旺旺短剧",
        "api": "https://wwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬优质资源",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬虎牙资源",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬鸭鸭资源",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬索尼资源",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬快车资源",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬闪电资源",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬樱花资源",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞麻豆视频",
        "api": "https://91md.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞--AIvin-",
        "api": "http://lbapiby.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞155-资源",
        "api": "https://155api.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞玉兔资源",
        "api": "https://apiyutu.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞番号资源",
        "api": "http://fhapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-老色逼-",
        "api": "https://apilsbzy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞优优资源",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://www.yytv4.cc/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞小鸡资源",
        "api": "https://api.xiaojizy.live/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黄色仓库",
        "api": "https://hsckzy.xyz/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-大奶子-",
        "api": "https://apidanaizi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞jkun资源",
        "api": "https://jkunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞乐播资源",
        "api": "https://lbapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奶香资源",
        "api": "https://Naixxzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞森林资源",
        "api": "https://beiyong.slapibf.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞辣椒资源",
        "api": "https://apilj.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞鲨鱼资源",
        "api": "https://shayuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞豆豆资源",
        "api": "https://api.douapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞滴滴资源",
        "api": "https://api.ddapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黑料资源",
        "api": "https://www.heiliaozyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞百万资源",
        "api": "https://api.bwzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞桃花资源",
        "api": "https://thzy1.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞精品资源",
        "api": "https://www.jingpinx.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞 CK-资源",
        "api": "https://ckzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞souavZY",
        "api": "https://api.souavzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞细胞资源",
        "api": "https://www.xxibaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞香蕉资源",
        "api": "https://www.xiangjiaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-美少女-",
        "api": "https://www.msnii.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-黄AVZY",
        "api": "https://www.pgxdy.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞白嫖资源",
        "api": "https://www.kxgav.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞杏吧资源",
        "api": "https://xingba222.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞大地资源",
        "api": "https://dadiapi.com/feifei",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🔞色猫资源",
        "api": "https://caiji.semaozy.net/inc/apijson_vod.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞-奥斯卡-",
        "api": "https://aosikazy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞丝袜资源",
        "api": "https://siwazyw.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      }
    ]
  },
  {
    "date": "2026-06-10",
    "keyword": "你好",
    "results": [
      {
        "name": "🎬-爱奇艺-",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豆瓣资源",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬天涯资源",
        "api": "https://tyyszy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬天涯影视",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬茅台资源",
        "api": "https://caiji.maotaizy.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬卧龙资源",
        "api": "https://wolongzyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬iKun资源",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬电影天堂",
        "api": "http://caiji.dyttzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子资源",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬360 资源",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬极速资源",
        "api": "https://jszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都资源",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬非凡资源",
        "api": "https://api.ffzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬暴风资源",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大资源",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽资源",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪资源",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬旺旺资源",
        "api": "https://api.wwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬速播资源",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰点播",
        "api": "https://www.jinyingzy.com/provide/vod?ct=xml",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬飘零资源",
        "api": "https://p2100.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷影视",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬光速资源",
        "api": "https://api.guangsuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛资源",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都动漫",
        "api": "https://caiji.moduapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬如意资源",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华资源",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云zy",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬1080资源",
        "api": "https://api.1080zyku.com/inc/apijson.php/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无水印资源",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬艾旦影视",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://lovedan.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子影视",
        "api": "https://cj.lziapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大点播",
        "api": "https://zuidazy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽影视",
        "api": "https://api.wujinapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬旺旺短剧",
        "api": "https://wwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬优质资源",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬虎牙资源",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬鸭鸭资源",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬索尼资源",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬快车资源",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬闪电资源",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬樱花资源",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞麻豆视频",
        "api": "https://91md.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞--AIvin-",
        "api": "http://lbapiby.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞155-资源",
        "api": "https://155api.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞玉兔资源",
        "api": "https://apiyutu.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞番号资源",
        "api": "http://fhapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-老色逼-",
        "api": "https://apilsbzy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞优优资源",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://www.yytv4.cc/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞小鸡资源",
        "api": "https://api.xiaojizy.live/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黄色仓库",
        "api": "https://hsckzy.xyz/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-大奶子-",
        "api": "https://apidanaizi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞jkun资源",
        "api": "https://jkunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞乐播资源",
        "api": "https://lbapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奶香资源",
        "api": "https://Naixxzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞森林资源",
        "api": "https://beiyong.slapibf.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞辣椒资源",
        "api": "https://apilj.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞鲨鱼资源",
        "api": "https://shayuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞豆豆资源",
        "api": "https://api.douapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞滴滴资源",
        "api": "https://api.ddapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黑料资源",
        "api": "https://www.heiliaozyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞百万资源",
        "api": "https://api.bwzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞桃花资源",
        "api": "https://thzy1.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞精品资源",
        "api": "https://www.jingpinx.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞 CK-资源",
        "api": "https://ckzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞souavZY",
        "api": "https://api.souavzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞细胞资源",
        "api": "https://www.xxibaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞香蕉资源",
        "api": "https://www.xiangjiaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-美少女-",
        "api": "https://www.msnii.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-黄AVZY",
        "api": "https://www.pgxdy.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞白嫖资源",
        "api": "https://www.kxgav.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞杏吧资源",
        "api": "https://xingba222.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞大地资源",
        "api": "https://dadiapi.com/feifei",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🔞色猫资源",
        "api": "https://caiji.semaozy.net/inc/apijson_vod.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞-奥斯卡-",
        "api": "https://aosikazy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞丝袜资源",
        "api": "https://siwazyw.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      }
    ]
  },
  {
    "date": "2026-06-11",
    "keyword": "你好",
    "results": [
      {
        "name": "🎬-爱奇艺-",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豆瓣资源",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬天涯资源",
        "api": "https://tyyszy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬天涯影视",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬茅台资源",
        "api": "https://caiji.maotaizy.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬卧龙资源",
        "api": "https://wolongzyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬iKun资源",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬电影天堂",
        "api": "http://caiji.dyttzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子资源",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬360 资源",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬极速资源",
        "api": "https://jszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都资源",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬非凡资源",
        "api": "https://api.ffzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬暴风资源",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大资源",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽资源",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪资源",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬旺旺资源",
        "api": "https://api.wwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬速播资源",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰点播",
        "api": "https://www.jinyingzy.com/provide/vod?ct=xml",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬飘零资源",
        "api": "https://p2100.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷影视",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬光速资源",
        "api": "https://api.guangsuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛资源",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都动漫",
        "api": "https://caiji.moduapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬如意资源",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华资源",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云zy",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬1080资源",
        "api": "https://api.1080zyku.com/inc/apijson.php/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无水印资源",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬艾旦影视",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://lovedan.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子影视",
        "api": "https://cj.lziapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大点播",
        "api": "https://zuidazy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽影视",
        "api": "https://api.wujinapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬旺旺短剧",
        "api": "https://wwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬优质资源",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬虎牙资源",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬鸭鸭资源",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬索尼资源",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬快车资源",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬闪电资源",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬樱花资源",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞麻豆视频",
        "api": "https://91md.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞--AIvin-",
        "api": "http://lbapiby.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞155-资源",
        "api": "https://155api.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞玉兔资源",
        "api": "https://apiyutu.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞番号资源",
        "api": "http://fhapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-老色逼-",
        "api": "https://apilsbzy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞优优资源",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://www.yytv4.cc/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞小鸡资源",
        "api": "https://api.xiaojizy.live/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黄色仓库",
        "api": "https://hsckzy.xyz/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-大奶子-",
        "api": "https://apidanaizi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞jkun资源",
        "api": "https://jkunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞乐播资源",
        "api": "https://lbapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奶香资源",
        "api": "https://Naixxzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞森林资源",
        "api": "https://beiyong.slapibf.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞辣椒资源",
        "api": "https://apilj.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞鲨鱼资源",
        "api": "https://shayuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞豆豆资源",
        "api": "https://api.douapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞滴滴资源",
        "api": "https://api.ddapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黑料资源",
        "api": "https://www.heiliaozyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞百万资源",
        "api": "https://api.bwzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞桃花资源",
        "api": "https://thzy1.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞精品资源",
        "api": "https://www.jingpinx.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞 CK-资源",
        "api": "https://ckzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞souavZY",
        "api": "https://api.souavzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞细胞资源",
        "api": "https://www.xxibaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞香蕉资源",
        "api": "https://www.xiangjiaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-美少女-",
        "api": "https://www.msnii.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-黄AVZY",
        "api": "https://www.pgxdy.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞白嫖资源",
        "api": "https://www.kxgav.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞杏吧资源",
        "api": "https://xingba222.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞大地资源",
        "api": "https://dadiapi.com/feifei",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🔞色猫资源",
        "api": "https://caiji.semaozy.net/inc/apijson_vod.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞-奥斯卡-",
        "api": "https://aosikazy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞丝袜资源",
        "api": "https://siwazyw.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      }
    ]
  },
  {
    "date": "2026-06-12",
    "keyword": "你好",
    "results": [
      {
        "name": "🎬-爱奇艺-",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豆瓣资源",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬天涯资源",
        "api": "https://tyyszy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬天涯影视",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬茅台资源",
        "api": "https://caiji.maotaizy.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬卧龙资源",
        "api": "https://wolongzyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬iKun资源",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬电影天堂",
        "api": "http://caiji.dyttzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子资源",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬360 资源",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬极速资源",
        "api": "https://jszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都资源",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬非凡资源",
        "api": "https://api.ffzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬暴风资源",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大资源",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽资源",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪资源",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬旺旺资源",
        "api": "https://api.wwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬速播资源",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰点播",
        "api": "https://www.jinyingzy.com/provide/vod?ct=xml",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬飘零资源",
        "api": "https://p2100.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷影视",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬光速资源",
        "api": "https://api.guangsuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛资源",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都动漫",
        "api": "https://caiji.moduapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬如意资源",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华资源",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云zy",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬1080资源",
        "api": "https://api.1080zyku.com/inc/apijson.php/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无水印资源",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬艾旦影视",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://lovedan.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子影视",
        "api": "https://cj.lziapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大点播",
        "api": "https://zuidazy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽影视",
        "api": "https://api.wujinapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬旺旺短剧",
        "api": "https://wwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬优质资源",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬虎牙资源",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬鸭鸭资源",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬索尼资源",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬快车资源",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬闪电资源",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬樱花资源",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞麻豆视频",
        "api": "https://91md.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞--AIvin-",
        "api": "http://lbapiby.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞155-资源",
        "api": "https://155api.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞玉兔资源",
        "api": "https://apiyutu.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞番号资源",
        "api": "http://fhapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-老色逼-",
        "api": "https://apilsbzy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞优优资源",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://www.yytv4.cc/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞小鸡资源",
        "api": "https://api.xiaojizy.live/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黄色仓库",
        "api": "https://hsckzy.xyz/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-大奶子-",
        "api": "https://apidanaizi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞jkun资源",
        "api": "https://jkunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞乐播资源",
        "api": "https://lbapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奶香资源",
        "api": "https://Naixxzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞森林资源",
        "api": "https://beiyong.slapibf.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞辣椒资源",
        "api": "https://apilj.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞鲨鱼资源",
        "api": "https://shayuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞豆豆资源",
        "api": "https://api.douapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞滴滴资源",
        "api": "https://api.ddapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黑料资源",
        "api": "https://www.heiliaozyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞百万资源",
        "api": "https://api.bwzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞桃花资源",
        "api": "https://thzy1.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞精品资源",
        "api": "https://www.jingpinx.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞 CK-资源",
        "api": "https://ckzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞souavZY",
        "api": "https://api.souavzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞细胞资源",
        "api": "https://www.xxibaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞香蕉资源",
        "api": "https://www.xiangjiaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-美少女-",
        "api": "https://www.msnii.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-黄AVZY",
        "api": "https://www.pgxdy.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞白嫖资源",
        "api": "https://www.kxgav.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞杏吧资源",
        "api": "https://xingba222.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞大地资源",
        "api": "https://dadiapi.com/feifei",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🔞色猫资源",
        "api": "https://caiji.semaozy.net/inc/apijson_vod.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞-奥斯卡-",
        "api": "https://aosikazy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞丝袜资源",
        "api": "https://siwazyw.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      }
    ]
  },
  {
    "date": "2026-06-13",
    "keyword": "你好",
    "results": [
      {
        "name": "🎬-爱奇艺-",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豆瓣资源",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬天涯资源",
        "api": "https://tyyszy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬天涯影视",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬茅台资源",
        "api": "https://caiji.maotaizy.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬卧龙资源",
        "api": "https://wolongzyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬iKun资源",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬电影天堂",
        "api": "http://caiji.dyttzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子资源",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬360 资源",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬极速资源",
        "api": "https://jszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都资源",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬非凡资源",
        "api": "https://api.ffzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬暴风资源",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大资源",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽资源",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪资源",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬旺旺资源",
        "api": "https://api.wwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬速播资源",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰点播",
        "api": "https://www.jinyingzy.com/provide/vod?ct=xml",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬飘零资源",
        "api": "https://p2100.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷影视",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬光速资源",
        "api": "https://api.guangsuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛资源",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都动漫",
        "api": "https://caiji.moduapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬如意资源",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华资源",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云zy",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬1080资源",
        "api": "https://api.1080zyku.com/inc/apijson.php/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无水印资源",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬艾旦影视",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://lovedan.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子影视",
        "api": "https://cj.lziapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大点播",
        "api": "https://zuidazy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽影视",
        "api": "https://api.wujinapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬旺旺短剧",
        "api": "https://wwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬优质资源",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬虎牙资源",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬鸭鸭资源",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬索尼资源",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬快车资源",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬闪电资源",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬樱花资源",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞麻豆视频",
        "api": "https://91md.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞--AIvin-",
        "api": "http://lbapiby.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞155-资源",
        "api": "https://155api.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞玉兔资源",
        "api": "https://apiyutu.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞番号资源",
        "api": "http://fhapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-老色逼-",
        "api": "https://apilsbzy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞优优资源",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://www.yytv4.cc/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞小鸡资源",
        "api": "https://api.xiaojizy.live/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黄色仓库",
        "api": "https://hsckzy.xyz/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-大奶子-",
        "api": "https://apidanaizi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞jkun资源",
        "api": "https://jkunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞乐播资源",
        "api": "https://lbapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奶香资源",
        "api": "https://Naixxzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞森林资源",
        "api": "https://beiyong.slapibf.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞辣椒资源",
        "api": "https://apilj.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞鲨鱼资源",
        "api": "https://shayuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞豆豆资源",
        "api": "https://api.douapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞滴滴资源",
        "api": "https://api.ddapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黑料资源",
        "api": "https://www.heiliaozyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞百万资源",
        "api": "https://api.bwzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞桃花资源",
        "api": "https://thzy1.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞精品资源",
        "api": "https://www.jingpinx.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞 CK-资源",
        "api": "https://ckzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞souavZY",
        "api": "https://api.souavzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞细胞资源",
        "api": "https://www.xxibaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞香蕉资源",
        "api": "https://www.xiangjiaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-美少女-",
        "api": "https://www.msnii.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-黄AVZY",
        "api": "https://www.pgxdy.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞白嫖资源",
        "api": "https://www.kxgav.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞杏吧资源",
        "api": "https://xingba222.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞大地资源",
        "api": "https://dadiapi.com/feifei",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🔞色猫资源",
        "api": "https://caiji.semaozy.net/inc/apijson_vod.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞-奥斯卡-",
        "api": "https://aosikazy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞丝袜资源",
        "api": "https://siwazyw.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      }
    ]
  },
  {
    "date": "2026-06-14",
    "keyword": "你好",
    "results": [
      {
        "name": "🎬-爱奇艺-",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豆瓣资源",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬天涯资源",
        "api": "https://tyyszy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬天涯影视",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬茅台资源",
        "api": "https://caiji.maotaizy.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬卧龙资源",
        "api": "https://wolongzyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬iKun资源",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬电影天堂",
        "api": "http://caiji.dyttzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子资源",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬360 资源",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬极速资源",
        "api": "https://jszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都资源",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬非凡资源",
        "api": "https://api.ffzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬暴风资源",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大资源",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽资源",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪资源",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬旺旺资源",
        "api": "https://api.wwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬速播资源",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰点播",
        "api": "https://www.jinyingzy.com/provide/vod?ct=xml",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬飘零资源",
        "api": "https://p2100.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷影视",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬光速资源",
        "api": "https://api.guangsuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛资源",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都动漫",
        "api": "https://caiji.moduapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬如意资源",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华资源",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云zy",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬1080资源",
        "api": "https://api.1080zyku.com/inc/apijson.php/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无水印资源",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬艾旦影视",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://lovedan.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子影视",
        "api": "https://cj.lziapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大点播",
        "api": "https://zuidazy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽影视",
        "api": "https://api.wujinapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬旺旺短剧",
        "api": "https://wwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬优质资源",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬虎牙资源",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬鸭鸭资源",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬索尼资源",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬快车资源",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬闪电资源",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬樱花资源",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞麻豆视频",
        "api": "https://91md.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞--AIvin-",
        "api": "http://lbapiby.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞155-资源",
        "api": "https://155api.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞玉兔资源",
        "api": "https://apiyutu.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞番号资源",
        "api": "http://fhapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-老色逼-",
        "api": "https://apilsbzy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞优优资源",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://www.yytv4.cc/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞小鸡资源",
        "api": "https://api.xiaojizy.live/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黄色仓库",
        "api": "https://hsckzy.xyz/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞-大奶子-",
        "api": "https://apidanaizi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞jkun资源",
        "api": "https://jkunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞乐播资源",
        "api": "https://lbapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奶香资源",
        "api": "https://Naixxzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞森林资源",
        "api": "https://beiyong.slapibf.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞辣椒资源",
        "api": "https://apilj.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞鲨鱼资源",
        "api": "https://shayuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞豆豆资源",
        "api": "https://api.douapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞滴滴资源",
        "api": "https://api.ddapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黑料资源",
        "api": "https://www.heiliaozyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞百万资源",
        "api": "https://api.bwzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞桃花资源",
        "api": "https://thzy1.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞精品资源",
        "api": "https://www.jingpinx.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞 CK-资源",
        "api": "https://ckzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞souavZY",
        "api": "https://api.souavzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞细胞资源",
        "api": "https://www.xxibaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞香蕉资源",
        "api": "https://www.xiangjiaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-美少女-",
        "api": "https://www.msnii.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-黄AVZY",
        "api": "https://www.pgxdy.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞白嫖资源",
        "api": "https://www.kxgav.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞杏吧资源",
        "api": "https://xingba222.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞大地资源",
        "api": "https://dadiapi.com/feifei",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🔞色猫资源",
        "api": "https://caiji.semaozy.net/inc/apijson_vod.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞-奥斯卡-",
        "api": "https://aosikazy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞丝袜资源",
        "api": "https://siwazyw.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      }
    ]
  },
  {
    "date": "2026-06-15",
    "keyword": "你好",
    "results": [
      {
        "name": "🎬-爱奇艺-",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豆瓣资源",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬天涯资源",
        "api": "https://tyyszy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬天涯影视",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬茅台资源",
        "api": "https://caiji.maotaizy.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬卧龙资源",
        "api": "https://wolongzyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬iKun资源",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬电影天堂",
        "api": "http://caiji.dyttzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子资源",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬360 资源",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬极速资源",
        "api": "https://jszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都资源",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬非凡资源",
        "api": "https://api.ffzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬暴风资源",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大资源",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽资源",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪资源",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬旺旺资源",
        "api": "https://api.wwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬速播资源",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰点播",
        "api": "https://www.jinyingzy.com/provide/vod?ct=xml",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬飘零资源",
        "api": "https://p2100.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷影视",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬光速资源",
        "api": "https://api.guangsuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛资源",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都动漫",
        "api": "https://caiji.moduapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬如意资源",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华资源",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云zy",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬1080资源",
        "api": "https://api.1080zyku.com/inc/apijson.php/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无水印资源",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬艾旦影视",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://lovedan.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子影视",
        "api": "https://cj.lziapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大点播",
        "api": "https://zuidazy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽影视",
        "api": "https://api.wujinapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬旺旺短剧",
        "api": "https://wwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬优质资源",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬虎牙资源",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬鸭鸭资源",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬索尼资源",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬快车资源",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬闪电资源",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬樱花资源",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞麻豆视频",
        "api": "https://91md.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞--AIvin-",
        "api": "http://lbapiby.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞155-资源",
        "api": "https://155api.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞玉兔资源",
        "api": "https://apiyutu.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞番号资源",
        "api": "http://fhapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-老色逼-",
        "api": "https://apilsbzy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞优优资源",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://www.yytv4.cc/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞小鸡资源",
        "api": "https://api.xiaojizy.live/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黄色仓库",
        "api": "https://hsckzy.xyz/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-大奶子-",
        "api": "https://apidanaizi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞jkun资源",
        "api": "https://jkunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞乐播资源",
        "api": "https://lbapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奶香资源",
        "api": "https://Naixxzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞森林资源",
        "api": "https://beiyong.slapibf.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞辣椒资源",
        "api": "https://apilj.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞鲨鱼资源",
        "api": "https://shayuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞豆豆资源",
        "api": "https://api.douapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞滴滴资源",
        "api": "https://api.ddapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黑料资源",
        "api": "https://www.heiliaozyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞百万资源",
        "api": "https://api.bwzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞桃花资源",
        "api": "https://thzy1.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞精品资源",
        "api": "https://www.jingpinx.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞 CK-资源",
        "api": "https://ckzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞souavZY",
        "api": "https://api.souavzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞细胞资源",
        "api": "https://www.xxibaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞香蕉资源",
        "api": "https://www.xiangjiaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-美少女-",
        "api": "https://www.msnii.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-黄AVZY",
        "api": "https://www.pgxdy.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞白嫖资源",
        "api": "https://www.kxgav.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞杏吧资源",
        "api": "https://xingba222.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞大地资源",
        "api": "https://dadiapi.com/feifei",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🔞色猫资源",
        "api": "https://caiji.semaozy.net/inc/apijson_vod.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞-奥斯卡-",
        "api": "https://aosikazy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞丝袜资源",
        "api": "https://siwazyw.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      }
    ]
  },
  {
    "date": "2026-06-16",
    "keyword": "你好",
    "results": [
      {
        "name": "🎬-爱奇艺-",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豆瓣资源",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬天涯资源",
        "api": "https://tyyszy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬天涯影视",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬茅台资源",
        "api": "https://caiji.maotaizy.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬卧龙资源",
        "api": "https://wolongzyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬iKun资源",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬电影天堂",
        "api": "http://caiji.dyttzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子资源",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬360 资源",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬极速资源",
        "api": "https://jszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都资源",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬非凡资源",
        "api": "https://api.ffzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬暴风资源",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大资源",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽资源",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪资源",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬旺旺资源",
        "api": "https://api.wwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬速播资源",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰点播",
        "api": "https://www.jinyingzy.com/provide/vod?ct=xml",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬飘零资源",
        "api": "https://p2100.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷影视",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬光速资源",
        "api": "https://api.guangsuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛资源",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都动漫",
        "api": "https://caiji.moduapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬如意资源",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华资源",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云zy",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬1080资源",
        "api": "https://api.1080zyku.com/inc/apijson.php/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无水印资源",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬艾旦影视",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://lovedan.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子影视",
        "api": "https://cj.lziapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大点播",
        "api": "https://zuidazy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽影视",
        "api": "https://api.wujinapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬旺旺短剧",
        "api": "https://wwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬优质资源",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬虎牙资源",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬鸭鸭资源",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬索尼资源",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬快车资源",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬闪电资源",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬樱花资源",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞麻豆视频",
        "api": "https://91md.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞--AIvin-",
        "api": "http://lbapiby.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞155-资源",
        "api": "https://155api.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞玉兔资源",
        "api": "https://apiyutu.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞番号资源",
        "api": "http://fhapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-老色逼-",
        "api": "https://apilsbzy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞优优资源",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://www.yytv4.cc/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞小鸡资源",
        "api": "https://api.xiaojizy.live/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黄色仓库",
        "api": "https://hsckzy.xyz/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞-大奶子-",
        "api": "https://apidanaizi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞jkun资源",
        "api": "https://jkunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞乐播资源",
        "api": "https://lbapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奶香资源",
        "api": "https://Naixxzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞森林资源",
        "api": "https://beiyong.slapibf.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞辣椒资源",
        "api": "https://apilj.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞鲨鱼资源",
        "api": "https://shayuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞豆豆资源",
        "api": "https://api.douapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞滴滴资源",
        "api": "https://api.ddapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黑料资源",
        "api": "https://www.heiliaozyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞百万资源",
        "api": "https://api.bwzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞桃花资源",
        "api": "https://thzy1.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞精品资源",
        "api": "https://www.jingpinx.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞 CK-资源",
        "api": "https://ckzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞souavZY",
        "api": "https://api.souavzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞细胞资源",
        "api": "https://www.xxibaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞香蕉资源",
        "api": "https://www.xiangjiaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-美少女-",
        "api": "https://www.msnii.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-黄AVZY",
        "api": "https://www.pgxdy.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞白嫖资源",
        "api": "https://www.kxgav.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞杏吧资源",
        "api": "https://xingba222.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞大地资源",
        "api": "https://dadiapi.com/feifei",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🔞色猫资源",
        "api": "https://caiji.semaozy.net/inc/apijson_vod.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞-奥斯卡-",
        "api": "https://aosikazy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞丝袜资源",
        "api": "https://siwazyw.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      }
    ]
  },
  {
    "date": "2026-06-17",
    "keyword": "你好",
    "results": [
      {
        "name": "🎬-爱奇艺-",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豆瓣资源",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬天涯资源",
        "api": "https://tyyszy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬天涯影视",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬茅台资源",
        "api": "https://caiji.maotaizy.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬卧龙资源",
        "api": "https://wolongzyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬iKun资源",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬电影天堂",
        "api": "http://caiji.dyttzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子资源",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬360 资源",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬极速资源",
        "api": "https://jszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都资源",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬非凡资源",
        "api": "https://api.ffzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬暴风资源",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大资源",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽资源",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪资源",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬旺旺资源",
        "api": "https://api.wwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬速播资源",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰点播",
        "api": "https://www.jinyingzy.com/provide/vod?ct=xml",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬飘零资源",
        "api": "https://p2100.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷影视",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬光速资源",
        "api": "https://api.guangsuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛资源",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都动漫",
        "api": "https://caiji.moduapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬如意资源",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华资源",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云zy",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬1080资源",
        "api": "https://api.1080zyku.com/inc/apijson.php/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无水印资源",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬艾旦影视",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://lovedan.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子影视",
        "api": "https://cj.lziapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大点播",
        "api": "https://zuidazy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽影视",
        "api": "https://api.wujinapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬旺旺短剧",
        "api": "https://wwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬优质资源",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬虎牙资源",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬鸭鸭资源",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬索尼资源",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬快车资源",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬闪电资源",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬樱花资源",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞麻豆视频",
        "api": "https://91md.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞--AIvin-",
        "api": "http://lbapiby.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞155-资源",
        "api": "https://155api.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞玉兔资源",
        "api": "https://apiyutu.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞番号资源",
        "api": "http://fhapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-老色逼-",
        "api": "https://apilsbzy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞优优资源",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://www.yytv4.cc/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞小鸡资源",
        "api": "https://api.xiaojizy.live/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黄色仓库",
        "api": "https://hsckzy.xyz/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞-大奶子-",
        "api": "https://apidanaizi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞jkun资源",
        "api": "https://jkunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞乐播资源",
        "api": "https://lbapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奶香资源",
        "api": "https://Naixxzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞森林资源",
        "api": "https://beiyong.slapibf.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞辣椒资源",
        "api": "https://apilj.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞鲨鱼资源",
        "api": "https://shayuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞豆豆资源",
        "api": "https://api.douapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞滴滴资源",
        "api": "https://api.ddapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黑料资源",
        "api": "https://www.heiliaozyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞百万资源",
        "api": "https://api.bwzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞桃花资源",
        "api": "https://thzy1.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞精品资源",
        "api": "https://www.jingpinx.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞 CK-资源",
        "api": "https://ckzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞souavZY",
        "api": "https://api.souavzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞细胞资源",
        "api": "https://www.xxibaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞香蕉资源",
        "api": "https://www.xiangjiaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-美少女-",
        "api": "https://www.msnii.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-黄AVZY",
        "api": "https://www.pgxdy.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞白嫖资源",
        "api": "https://www.kxgav.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞杏吧资源",
        "api": "https://xingba222.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞大地资源",
        "api": "https://dadiapi.com/feifei",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🔞色猫资源",
        "api": "https://caiji.semaozy.net/inc/apijson_vod.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞-奥斯卡-",
        "api": "https://aosikazy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞丝袜资源",
        "api": "https://siwazyw.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      }
    ]
  },
  {
    "date": "2026-06-18",
    "keyword": "你好",
    "results": [
      {
        "name": "🎬-爱奇艺-",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豆瓣资源",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬天涯资源",
        "api": "https://tyyszy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬天涯影视",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬茅台资源",
        "api": "https://caiji.maotaizy.cc/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬卧龙资源",
        "api": "https://wolongzyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬iKun资源",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬电影天堂",
        "api": "http://caiji.dyttzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子资源",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬360 资源",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬极速资源",
        "api": "https://jszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都资源",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬非凡资源",
        "api": "https://api.ffzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬暴风资源",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大资源",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽资源",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪资源",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬旺旺资源",
        "api": "https://api.wwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬速播资源",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰点播",
        "api": "https://www.jinyingzy.com/provide/vod?ct=xml",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬飘零资源",
        "api": "https://p2100.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷影视",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬光速资源",
        "api": "https://api.guangsuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛资源",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都动漫",
        "api": "https://caiji.moduapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬如意资源",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华资源",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云zy",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬1080资源",
        "api": "https://api.1080zyku.com/inc/apijson.php/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无水印资源",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬艾旦影视",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://lovedan.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子影视",
        "api": "https://cj.lziapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大点播",
        "api": "https://zuidazy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽影视",
        "api": "https://api.wujinapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬旺旺短剧",
        "api": "https://wwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬优质资源",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬虎牙资源",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬鸭鸭资源",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬索尼资源",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬快车资源",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬闪电资源",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬樱花资源",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞麻豆视频",
        "api": "https://91md.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞--AIvin-",
        "api": "http://lbapiby.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞155-资源",
        "api": "https://155api.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞玉兔资源",
        "api": "https://apiyutu.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞番号资源",
        "api": "http://fhapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-老色逼-",
        "api": "https://apilsbzy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞优优资源",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://www.yytv4.cc/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞小鸡资源",
        "api": "https://api.xiaojizy.live/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黄色仓库",
        "api": "https://hsckzy.xyz/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-大奶子-",
        "api": "https://apidanaizi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞jkun资源",
        "api": "https://jkunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞乐播资源",
        "api": "https://lbapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奶香资源",
        "api": "https://Naixxzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞森林资源",
        "api": "https://beiyong.slapibf.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞辣椒资源",
        "api": "https://apilj.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞鲨鱼资源",
        "api": "https://shayuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞豆豆资源",
        "api": "https://api.douapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞滴滴资源",
        "api": "https://api.ddapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黑料资源",
        "api": "https://www.heiliaozyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞百万资源",
        "api": "https://api.bwzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞桃花资源",
        "api": "https://thzy1.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞精品资源",
        "api": "https://www.jingpinx.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞 CK-资源",
        "api": "https://ckzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞souavZY",
        "api": "https://api.souavzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞细胞资源",
        "api": "https://www.xxibaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞香蕉资源",
        "api": "https://www.xiangjiaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-美少女-",
        "api": "https://www.msnii.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-黄AVZY",
        "api": "https://www.pgxdy.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞白嫖资源",
        "api": "https://www.kxgav.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞杏吧资源",
        "api": "https://xingba222.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞大地资源",
        "api": "https://dadiapi.com/feifei",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🔞色猫资源",
        "api": "https://caiji.semaozy.net/inc/apijson_vod.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞-奥斯卡-",
        "api": "https://aosikazy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞丝袜资源",
        "api": "https://siwazyw.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      }
    ]
  },
  {
    "date": "2026-06-19",
    "keyword": "你好",
    "results": [
      {
        "name": "🎬-爱奇艺-",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豆瓣资源",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬天涯资源",
        "api": "https://tyyszy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬天涯影视",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬茅台资源",
        "api": "https://caiji.maotaizy.cc/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬卧龙资源",
        "api": "https://wolongzyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬iKun资源",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬电影天堂",
        "api": "http://caiji.dyttzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子资源",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬360 资源",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬极速资源",
        "api": "https://jszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都资源",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬非凡资源",
        "api": "https://api.ffzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬暴风资源",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大资源",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽资源",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪资源",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬旺旺资源",
        "api": "https://api.wwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬速播资源",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰点播",
        "api": "https://www.jinyingzy.com/provide/vod?ct=xml",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬飘零资源",
        "api": "https://p2100.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷影视",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬光速资源",
        "api": "https://api.guangsuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛资源",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都动漫",
        "api": "https://caiji.moduapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬如意资源",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华资源",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云zy",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬1080资源",
        "api": "https://api.1080zyku.com/inc/apijson.php/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无水印资源",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬艾旦影视",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://lovedan.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子影视",
        "api": "https://cj.lziapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大点播",
        "api": "https://zuidazy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽影视",
        "api": "https://api.wujinapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬旺旺短剧",
        "api": "https://wwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬优质资源",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬虎牙资源",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬鸭鸭资源",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬索尼资源",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬快车资源",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬闪电资源",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬樱花资源",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞麻豆视频",
        "api": "https://91md.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞--AIvin-",
        "api": "http://lbapiby.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞155-资源",
        "api": "https://155api.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞玉兔资源",
        "api": "https://apiyutu.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞番号资源",
        "api": "http://fhapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-老色逼-",
        "api": "https://apilsbzy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞优优资源",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://www.yytv4.cc/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞小鸡资源",
        "api": "https://api.xiaojizy.live/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黄色仓库",
        "api": "https://hsckzy.xyz/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-大奶子-",
        "api": "https://apidanaizi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞jkun资源",
        "api": "https://jkunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞乐播资源",
        "api": "https://lbapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奶香资源",
        "api": "https://Naixxzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞森林资源",
        "api": "https://beiyong.slapibf.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞辣椒资源",
        "api": "https://apilj.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞鲨鱼资源",
        "api": "https://shayuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞豆豆资源",
        "api": "https://api.douapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞滴滴资源",
        "api": "https://api.ddapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黑料资源",
        "api": "https://www.heiliaozyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞百万资源",
        "api": "https://api.bwzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞桃花资源",
        "api": "https://thzy1.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞精品资源",
        "api": "https://www.jingpinx.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞 CK-资源",
        "api": "https://ckzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞souavZY",
        "api": "https://api.souavzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞细胞资源",
        "api": "https://www.xxibaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞香蕉资源",
        "api": "https://www.xiangjiaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-美少女-",
        "api": "https://www.msnii.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-黄AVZY",
        "api": "https://www.pgxdy.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞白嫖资源",
        "api": "https://www.kxgav.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞杏吧资源",
        "api": "https://xingba222.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞大地资源",
        "api": "https://dadiapi.com/feifei",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🔞色猫资源",
        "api": "https://caiji.semaozy.net/inc/apijson_vod.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞-奥斯卡-",
        "api": "https://aosikazy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞丝袜资源",
        "api": "https://siwazyw.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      }
    ]
  },
  {
    "date": "2026-06-20",
    "keyword": "你好",
    "results": [
      {
        "name": "🎬-爱奇艺-",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豆瓣资源",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬天涯资源",
        "api": "https://tyyszy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬天涯影视",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬茅台资源",
        "api": "https://caiji.maotaizy.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬卧龙资源",
        "api": "https://wolongzyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬iKun资源",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬电影天堂",
        "api": "http://caiji.dyttzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子资源",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬360 资源",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬极速资源",
        "api": "https://jszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都资源",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬非凡资源",
        "api": "https://api.ffzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬暴风资源",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大资源",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽资源",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪资源",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬旺旺资源",
        "api": "https://api.wwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬速播资源",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰点播",
        "api": "https://www.jinyingzy.com/provide/vod?ct=xml",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬飘零资源",
        "api": "https://p2100.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷影视",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬光速资源",
        "api": "https://api.guangsuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛资源",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都动漫",
        "api": "https://caiji.moduapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬如意资源",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华资源",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云zy",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬1080资源",
        "api": "https://api.1080zyku.com/inc/apijson.php/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无水印资源",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬艾旦影视",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://lovedan.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子影视",
        "api": "https://cj.lziapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大点播",
        "api": "https://zuidazy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽影视",
        "api": "https://api.wujinapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬旺旺短剧",
        "api": "https://wwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬优质资源",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬虎牙资源",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬鸭鸭资源",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬索尼资源",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬快车资源",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬闪电资源",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬樱花资源",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞麻豆视频",
        "api": "https://91md.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞--AIvin-",
        "api": "http://lbapiby.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞155-资源",
        "api": "https://155api.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞玉兔资源",
        "api": "https://apiyutu.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞番号资源",
        "api": "http://fhapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-老色逼-",
        "api": "https://apilsbzy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞优优资源",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://www.yytv4.cc/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞小鸡资源",
        "api": "https://api.xiaojizy.live/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黄色仓库",
        "api": "https://hsckzy.xyz/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-大奶子-",
        "api": "https://apidanaizi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞jkun资源",
        "api": "https://jkunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞乐播资源",
        "api": "https://lbapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奶香资源",
        "api": "https://Naixxzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞森林资源",
        "api": "https://beiyong.slapibf.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞辣椒资源",
        "api": "https://apilj.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞鲨鱼资源",
        "api": "https://shayuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞豆豆资源",
        "api": "https://api.douapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞滴滴资源",
        "api": "https://api.ddapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黑料资源",
        "api": "https://www.heiliaozyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞百万资源",
        "api": "https://api.bwzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞桃花资源",
        "api": "https://thzy1.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞精品资源",
        "api": "https://www.jingpinx.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞 CK-资源",
        "api": "https://ckzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞souavZY",
        "api": "https://api.souavzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞细胞资源",
        "api": "https://www.xxibaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞香蕉资源",
        "api": "https://www.xiangjiaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-美少女-",
        "api": "https://www.msnii.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-黄AVZY",
        "api": "https://www.pgxdy.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞白嫖资源",
        "api": "https://www.kxgav.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞杏吧资源",
        "api": "https://xingba222.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞大地资源",
        "api": "https://dadiapi.com/feifei",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🔞色猫资源",
        "api": "https://caiji.semaozy.net/inc/apijson_vod.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞-奥斯卡-",
        "api": "https://aosikazy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞丝袜资源",
        "api": "https://siwazyw.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      }
    ]
  },
  {
    "date": "2026-06-21",
    "keyword": "你好",
    "results": [
      {
        "name": "🎬-爱奇艺-",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豆瓣资源",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬天涯资源",
        "api": "https://tyyszy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬天涯影视",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬茅台资源",
        "api": "https://caiji.maotaizy.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬卧龙资源",
        "api": "https://wolongzyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬iKun资源",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬电影天堂",
        "api": "http://caiji.dyttzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子资源",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬360 资源",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬极速资源",
        "api": "https://jszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都资源",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬非凡资源",
        "api": "https://api.ffzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬暴风资源",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大资源",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽资源",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪资源",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬旺旺资源",
        "api": "https://api.wwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬速播资源",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰点播",
        "api": "https://www.jinyingzy.com/provide/vod?ct=xml",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬飘零资源",
        "api": "https://p2100.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷影视",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬光速资源",
        "api": "https://api.guangsuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛资源",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都动漫",
        "api": "https://caiji.moduapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬如意资源",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华资源",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云zy",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬1080资源",
        "api": "https://api.1080zyku.com/inc/apijson.php/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无水印资源",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬艾旦影视",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://lovedan.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子影视",
        "api": "https://cj.lziapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大点播",
        "api": "https://zuidazy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽影视",
        "api": "https://api.wujinapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬旺旺短剧",
        "api": "https://wwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬优质资源",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬虎牙资源",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬鸭鸭资源",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬索尼资源",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬快车资源",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬闪电资源",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬樱花资源",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞麻豆视频",
        "api": "https://91md.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞--AIvin-",
        "api": "http://lbapiby.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞155-资源",
        "api": "https://155api.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞玉兔资源",
        "api": "https://apiyutu.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞番号资源",
        "api": "http://fhapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-老色逼-",
        "api": "https://apilsbzy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞优优资源",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://www.yytv4.cc/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞小鸡资源",
        "api": "https://api.xiaojizy.live/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黄色仓库",
        "api": "https://hsckzy.xyz/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-大奶子-",
        "api": "https://apidanaizi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞jkun资源",
        "api": "https://jkunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞乐播资源",
        "api": "https://lbapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奶香资源",
        "api": "https://Naixxzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞森林资源",
        "api": "https://beiyong.slapibf.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞辣椒资源",
        "api": "https://apilj.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞鲨鱼资源",
        "api": "https://shayuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞豆豆资源",
        "api": "https://api.douapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞滴滴资源",
        "api": "https://api.ddapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黑料资源",
        "api": "https://www.heiliaozyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞百万资源",
        "api": "https://api.bwzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞桃花资源",
        "api": "https://thzy1.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞精品资源",
        "api": "https://www.jingpinx.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞 CK-资源",
        "api": "https://ckzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞souavZY",
        "api": "https://api.souavzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞细胞资源",
        "api": "https://www.xxibaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞香蕉资源",
        "api": "https://www.xiangjiaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-美少女-",
        "api": "https://www.msnii.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-黄AVZY",
        "api": "https://www.pgxdy.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞白嫖资源",
        "api": "https://www.kxgav.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞杏吧资源",
        "api": "https://xingba222.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞大地资源",
        "api": "https://dadiapi.com/feifei",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🔞色猫资源",
        "api": "https://caiji.semaozy.net/inc/apijson_vod.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞-奥斯卡-",
        "api": "https://aosikazy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞丝袜资源",
        "api": "https://siwazyw.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      }
    ]
  },
  {
    "date": "2026-06-22",
    "keyword": "你好",
    "results": [
      {
        "name": "🎬-爱奇艺-",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豆瓣资源",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬天涯资源",
        "api": "https://tyyszy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬天涯影视",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬茅台资源",
        "api": "https://caiji.maotaizy.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬卧龙资源",
        "api": "https://wolongzyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬iKun资源",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬电影天堂",
        "api": "http://caiji.dyttzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子资源",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬360 资源",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬极速资源",
        "api": "https://jszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都资源",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬非凡资源",
        "api": "https://api.ffzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬暴风资源",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大资源",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽资源",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪资源",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬旺旺资源",
        "api": "https://api.wwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬速播资源",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰点播",
        "api": "https://www.jinyingzy.com/provide/vod?ct=xml",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬飘零资源",
        "api": "https://p2100.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷影视",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬光速资源",
        "api": "https://api.guangsuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛资源",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都动漫",
        "api": "https://caiji.moduapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬如意资源",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华资源",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云zy",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬1080资源",
        "api": "https://api.1080zyku.com/inc/apijson.php/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无水印资源",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬艾旦影视",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://lovedan.net/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬量子影视",
        "api": "https://cj.lziapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大点播",
        "api": "https://zuidazy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽影视",
        "api": "https://api.wujinapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬旺旺短剧",
        "api": "https://wwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬优质资源",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬虎牙资源",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬鸭鸭资源",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬索尼资源",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬快车资源",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬闪电资源",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬樱花资源",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞麻豆视频",
        "api": "https://91md.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞--AIvin-",
        "api": "http://lbapiby.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞155-资源",
        "api": "https://155api.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞玉兔资源",
        "api": "https://apiyutu.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞番号资源",
        "api": "http://fhapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-老色逼-",
        "api": "https://apilsbzy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞优优资源",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://www.yytv4.cc/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞小鸡资源",
        "api": "https://api.xiaojizy.live/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黄色仓库",
        "api": "https://hsckzy.xyz/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞-大奶子-",
        "api": "https://apidanaizi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞jkun资源",
        "api": "https://jkunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞乐播资源",
        "api": "https://lbapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奶香资源",
        "api": "https://Naixxzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞森林资源",
        "api": "https://beiyong.slapibf.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞辣椒资源",
        "api": "https://apilj.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞鲨鱼资源",
        "api": "https://shayuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞豆豆资源",
        "api": "https://api.douapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞滴滴资源",
        "api": "https://api.ddapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黑料资源",
        "api": "https://www.heiliaozyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞百万资源",
        "api": "https://api.bwzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞桃花资源",
        "api": "https://thzy1.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞精品资源",
        "api": "https://www.jingpinx.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞 CK-资源",
        "api": "https://ckzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞souavZY",
        "api": "https://api.souavzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞细胞资源",
        "api": "https://www.xxibaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞香蕉资源",
        "api": "https://www.xiangjiaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-美少女-",
        "api": "https://www.msnii.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-黄AVZY",
        "api": "https://www.pgxdy.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞白嫖资源",
        "api": "https://www.kxgav.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞杏吧资源",
        "api": "https://xingba222.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞大地资源",
        "api": "https://dadiapi.com/feifei",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🔞色猫资源",
        "api": "https://caiji.semaozy.net/inc/apijson_vod.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞-奥斯卡-",
        "api": "https://aosikazy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞丝袜资源",
        "api": "https://siwazyw.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      }
    ]
  },
  {
    "date": "2026-06-23",
    "keyword": "你好",
    "results": [
      {
        "name": "🎬-爱奇艺-",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豆瓣资源",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬天涯资源",
        "api": "https://tyyszy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬天涯影视",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬茅台资源",
        "api": "https://caiji.maotaizy.cc/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬卧龙资源",
        "api": "https://wolongzyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬iKun资源",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬电影天堂",
        "api": "http://caiji.dyttzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子资源",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬360 资源",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬极速资源",
        "api": "https://jszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都资源",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬非凡资源",
        "api": "https://api.ffzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬暴风资源",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大资源",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽资源",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪资源",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬旺旺资源",
        "api": "https://api.wwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬速播资源",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰点播",
        "api": "https://www.jinyingzy.com/provide/vod?ct=xml",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬飘零资源",
        "api": "https://p2100.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷影视",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬光速资源",
        "api": "https://api.guangsuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛资源",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都动漫",
        "api": "https://caiji.moduapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬如意资源",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华资源",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云zy",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬1080资源",
        "api": "https://api.1080zyku.com/inc/apijson.php/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无水印资源",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬艾旦影视",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://lovedan.net/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬量子影视",
        "api": "https://cj.lziapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大点播",
        "api": "https://zuidazy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽影视",
        "api": "https://api.wujinapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬旺旺短剧",
        "api": "https://wwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬优质资源",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬虎牙资源",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬鸭鸭资源",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬索尼资源",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬快车资源",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬闪电资源",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬樱花资源",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞麻豆视频",
        "api": "https://91md.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞--AIvin-",
        "api": "http://lbapiby.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞155-资源",
        "api": "https://155api.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞玉兔资源",
        "api": "https://apiyutu.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞番号资源",
        "api": "http://fhapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-老色逼-",
        "api": "https://apilsbzy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞优优资源",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://www.yytv4.cc/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞小鸡资源",
        "api": "https://api.xiaojizy.live/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黄色仓库",
        "api": "https://hsckzy.xyz/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-大奶子-",
        "api": "https://apidanaizi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞jkun资源",
        "api": "https://jkunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞乐播资源",
        "api": "https://lbapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奶香资源",
        "api": "https://Naixxzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞森林资源",
        "api": "https://beiyong.slapibf.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞辣椒资源",
        "api": "https://apilj.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞鲨鱼资源",
        "api": "https://shayuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞豆豆资源",
        "api": "https://api.douapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞滴滴资源",
        "api": "https://api.ddapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黑料资源",
        "api": "https://www.heiliaozyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞百万资源",
        "api": "https://api.bwzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞桃花资源",
        "api": "https://thzy1.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞精品资源",
        "api": "https://www.jingpinx.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞 CK-资源",
        "api": "https://ckzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞souavZY",
        "api": "https://api.souavzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞细胞资源",
        "api": "https://www.xxibaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞香蕉资源",
        "api": "https://www.xiangjiaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-美少女-",
        "api": "https://www.msnii.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-黄AVZY",
        "api": "https://www.pgxdy.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞白嫖资源",
        "api": "https://www.kxgav.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞杏吧资源",
        "api": "https://xingba222.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞大地资源",
        "api": "https://dadiapi.com/feifei",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🔞色猫资源",
        "api": "https://caiji.semaozy.net/inc/apijson_vod.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞-奥斯卡-",
        "api": "https://aosikazy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞丝袜资源",
        "api": "https://siwazyw.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      }
    ]
  },
  {
    "date": "2026-06-24",
    "keyword": "你好",
    "results": [
      {
        "name": "🎬-爱奇艺-",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豆瓣资源",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🎬天涯资源",
        "api": "https://tyyszy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬天涯影视",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬茅台资源",
        "api": "https://caiji.maotaizy.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🎬卧龙资源",
        "api": "https://wolongzyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬iKun资源",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬电影天堂",
        "api": "http://caiji.dyttzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子资源",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬360 资源",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬极速资源",
        "api": "https://jszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都资源",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬非凡资源",
        "api": "https://api.ffzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬暴风资源",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大资源",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽资源",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪资源",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬旺旺资源",
        "api": "https://api.wwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬速播资源",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰点播",
        "api": "https://www.jinyingzy.com/provide/vod?ct=xml",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬飘零资源",
        "api": "https://p2100.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷影视",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬光速资源",
        "api": "https://api.guangsuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛资源",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都动漫",
        "api": "https://caiji.moduapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬如意资源",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华资源",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云zy",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬1080资源",
        "api": "https://api.1080zyku.com/inc/apijson.php/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无水印资源",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬艾旦影视",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://lovedan.net/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬量子影视",
        "api": "https://cj.lziapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大点播",
        "api": "https://zuidazy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽影视",
        "api": "https://api.wujinapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬旺旺短剧",
        "api": "https://wwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬优质资源",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬虎牙资源",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬鸭鸭资源",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬索尼资源",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬快车资源",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬闪电资源",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬樱花资源",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞麻豆视频",
        "api": "https://91md.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞--AIvin-",
        "api": "http://lbapiby.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞155-资源",
        "api": "https://155api.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞玉兔资源",
        "api": "https://apiyutu.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞番号资源",
        "api": "http://fhapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-老色逼-",
        "api": "https://apilsbzy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞优优资源",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://www.yytv4.cc/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞小鸡资源",
        "api": "https://api.xiaojizy.live/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黄色仓库",
        "api": "https://hsckzy.xyz/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-大奶子-",
        "api": "https://apidanaizi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞jkun资源",
        "api": "https://jkunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞乐播资源",
        "api": "https://lbapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奶香资源",
        "api": "https://Naixxzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞森林资源",
        "api": "https://beiyong.slapibf.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞辣椒资源",
        "api": "https://apilj.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞鲨鱼资源",
        "api": "https://shayuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞豆豆资源",
        "api": "https://api.douapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞滴滴资源",
        "api": "https://api.ddapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黑料资源",
        "api": "https://www.heiliaozyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞百万资源",
        "api": "https://api.bwzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞桃花资源",
        "api": "https://thzy1.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞精品资源",
        "api": "https://www.jingpinx.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞 CK-资源",
        "api": "https://ckzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞souavZY",
        "api": "https://api.souavzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞细胞资源",
        "api": "https://www.xxibaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞香蕉资源",
        "api": "https://www.xiangjiaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-美少女-",
        "api": "https://www.msnii.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-黄AVZY",
        "api": "https://www.pgxdy.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞白嫖资源",
        "api": "https://www.kxgav.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞杏吧资源",
        "api": "https://xingba222.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞大地资源",
        "api": "https://dadiapi.com/feifei",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🔞色猫资源",
        "api": "https://caiji.semaozy.net/inc/apijson_vod.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞-奥斯卡-",
        "api": "https://aosikazy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞丝袜资源",
        "api": "https://siwazyw.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      }
    ]
  },
  {
    "date": "2026-06-25",
    "keyword": "你好",
    "results": [
      {
        "name": "🎬-爱奇艺-",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豆瓣资源",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🎬天涯资源",
        "api": "https://tyyszy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬天涯影视",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬茅台资源",
        "api": "https://caiji.maotaizy.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🎬卧龙资源",
        "api": "https://wolongzyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬iKun资源",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬电影天堂",
        "api": "http://caiji.dyttzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子资源",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬360 资源",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬极速资源",
        "api": "https://jszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都资源",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬非凡资源",
        "api": "https://api.ffzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬暴风资源",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大资源",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽资源",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪资源",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬旺旺资源",
        "api": "https://api.wwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬速播资源",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰点播",
        "api": "https://www.jinyingzy.com/provide/vod?ct=xml",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬飘零资源",
        "api": "https://p2100.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷影视",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬光速资源",
        "api": "https://api.guangsuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛资源",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都动漫",
        "api": "https://caiji.moduapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬如意资源",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华资源",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云zy",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬1080资源",
        "api": "https://api.1080zyku.com/inc/apijson.php/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无水印资源",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬艾旦影视",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://lovedan.net/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬量子影视",
        "api": "https://cj.lziapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大点播",
        "api": "https://zuidazy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽影视",
        "api": "https://api.wujinapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬旺旺短剧",
        "api": "https://wwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬优质资源",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬虎牙资源",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬鸭鸭资源",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬索尼资源",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬快车资源",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬闪电资源",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬樱花资源",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞麻豆视频",
        "api": "https://91md.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞--AIvin-",
        "api": "http://lbapiby.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞155-资源",
        "api": "https://155api.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞玉兔资源",
        "api": "https://apiyutu.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞番号资源",
        "api": "http://fhapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-老色逼-",
        "api": "https://apilsbzy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞优优资源",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://www.yytv4.cc/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞小鸡资源",
        "api": "https://api.xiaojizy.live/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黄色仓库",
        "api": "https://hsckzy.xyz/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-大奶子-",
        "api": "https://apidanaizi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞jkun资源",
        "api": "https://jkunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞乐播资源",
        "api": "https://lbapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奶香资源",
        "api": "https://Naixxzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞森林资源",
        "api": "https://beiyong.slapibf.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞辣椒资源",
        "api": "https://apilj.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞鲨鱼资源",
        "api": "https://shayuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞豆豆资源",
        "api": "https://api.douapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞滴滴资源",
        "api": "https://api.ddapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黑料资源",
        "api": "https://www.heiliaozyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞百万资源",
        "api": "https://api.bwzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞桃花资源",
        "api": "https://thzy1.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞精品资源",
        "api": "https://www.jingpinx.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞 CK-资源",
        "api": "https://ckzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞souavZY",
        "api": "https://api.souavzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞细胞资源",
        "api": "https://www.xxibaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞香蕉资源",
        "api": "https://www.xiangjiaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-美少女-",
        "api": "https://www.msnii.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-黄AVZY",
        "api": "https://www.pgxdy.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞白嫖资源",
        "api": "https://www.kxgav.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞杏吧资源",
        "api": "https://xingba222.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞大地资源",
        "api": "https://dadiapi.com/feifei",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🔞色猫资源",
        "api": "https://caiji.semaozy.net/inc/apijson_vod.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞-奥斯卡-",
        "api": "https://aosikazy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞丝袜资源",
        "api": "https://siwazyw.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      }
    ]
  },
  {
    "date": "2026-06-26",
    "keyword": "你好",
    "results": [
      {
        "name": "🎬-爱奇艺-",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豆瓣资源",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🎬天涯资源",
        "api": "https://tyyszy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬天涯影视",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬茅台资源",
        "api": "https://caiji.maotaizy.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🎬卧龙资源",
        "api": "https://wolongzyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬iKun资源",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬电影天堂",
        "api": "http://caiji.dyttzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子资源",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬360 资源",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬极速资源",
        "api": "https://jszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都资源",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬非凡资源",
        "api": "https://api.ffzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬暴风资源",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大资源",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽资源",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪资源",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬旺旺资源",
        "api": "https://api.wwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬速播资源",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰点播",
        "api": "https://www.jinyingzy.com/provide/vod?ct=xml",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬飘零资源",
        "api": "https://p2100.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷影视",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬光速资源",
        "api": "https://api.guangsuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛资源",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都动漫",
        "api": "https://caiji.moduapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬如意资源",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华资源",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云zy",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬1080资源",
        "api": "https://api.1080zyku.com/inc/apijson.php/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无水印资源",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬艾旦影视",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://lovedan.net/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬量子影视",
        "api": "https://cj.lziapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大点播",
        "api": "https://zuidazy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽影视",
        "api": "https://api.wujinapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬旺旺短剧",
        "api": "https://wwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬优质资源",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬虎牙资源",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬鸭鸭资源",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬索尼资源",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬快车资源",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬闪电资源",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬樱花资源",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞麻豆视频",
        "api": "https://91md.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞--AIvin-",
        "api": "http://lbapiby.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞155-资源",
        "api": "https://155api.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞玉兔资源",
        "api": "https://apiyutu.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞番号资源",
        "api": "http://fhapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-老色逼-",
        "api": "https://apilsbzy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞优优资源",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://www.yytv4.cc/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞小鸡资源",
        "api": "https://api.xiaojizy.live/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黄色仓库",
        "api": "https://hsckzy.xyz/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-大奶子-",
        "api": "https://apidanaizi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞jkun资源",
        "api": "https://jkunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞乐播资源",
        "api": "https://lbapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奶香资源",
        "api": "https://Naixxzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞森林资源",
        "api": "https://beiyong.slapibf.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞辣椒资源",
        "api": "https://apilj.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞鲨鱼资源",
        "api": "https://shayuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞豆豆资源",
        "api": "https://api.douapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞滴滴资源",
        "api": "https://api.ddapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黑料资源",
        "api": "https://www.heiliaozyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞百万资源",
        "api": "https://api.bwzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞桃花资源",
        "api": "https://thzy1.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞精品资源",
        "api": "https://www.jingpinx.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞 CK-资源",
        "api": "https://ckzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞souavZY",
        "api": "https://api.souavzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞细胞资源",
        "api": "https://www.xxibaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞香蕉资源",
        "api": "https://www.xiangjiaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-美少女-",
        "api": "https://www.msnii.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-黄AVZY",
        "api": "https://www.pgxdy.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞白嫖资源",
        "api": "https://www.kxgav.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞杏吧资源",
        "api": "https://xingba222.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞大地资源",
        "api": "https://dadiapi.com/feifei",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🔞色猫资源",
        "api": "https://caiji.semaozy.net/inc/apijson_vod.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞-奥斯卡-",
        "api": "https://aosikazy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞丝袜资源",
        "api": "https://siwazyw.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      }
    ]
  },
  {
    "date": "2026-06-27",
    "keyword": "你好",
    "results": [
      {
        "name": "🎬-爱奇艺-",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豆瓣资源",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🎬天涯资源",
        "api": "https://tyyszy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬天涯影视",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬茅台资源",
        "api": "https://caiji.maotaizy.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🎬卧龙资源",
        "api": "https://wolongzyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬iKun资源",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬电影天堂",
        "api": "http://caiji.dyttzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子资源",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬360 资源",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬极速资源",
        "api": "https://jszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都资源",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬非凡资源",
        "api": "https://api.ffzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬暴风资源",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大资源",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽资源",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪资源",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬旺旺资源",
        "api": "https://api.wwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬速播资源",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰点播",
        "api": "https://www.jinyingzy.com/provide/vod?ct=xml",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬飘零资源",
        "api": "https://p2100.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷影视",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬光速资源",
        "api": "https://api.guangsuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛资源",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都动漫",
        "api": "https://caiji.moduapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬如意资源",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华资源",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云zy",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬1080资源",
        "api": "https://api.1080zyku.com/inc/apijson.php/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无水印资源",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬艾旦影视",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://lovedan.net/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬量子影视",
        "api": "https://cj.lziapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大点播",
        "api": "https://zuidazy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽影视",
        "api": "https://api.wujinapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬旺旺短剧",
        "api": "https://wwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬优质资源",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬虎牙资源",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬鸭鸭资源",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬索尼资源",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬快车资源",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬闪电资源",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬樱花资源",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞麻豆视频",
        "api": "https://91md.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞--AIvin-",
        "api": "http://lbapiby.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞155-资源",
        "api": "https://155api.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞玉兔资源",
        "api": "https://apiyutu.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞番号资源",
        "api": "http://fhapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-老色逼-",
        "api": "https://apilsbzy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞优优资源",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://www.yytv4.cc/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞小鸡资源",
        "api": "https://api.xiaojizy.live/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黄色仓库",
        "api": "https://hsckzy.xyz/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-大奶子-",
        "api": "https://apidanaizi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞jkun资源",
        "api": "https://jkunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞乐播资源",
        "api": "https://lbapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奶香资源",
        "api": "https://Naixxzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞森林资源",
        "api": "https://beiyong.slapibf.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞辣椒资源",
        "api": "https://apilj.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞鲨鱼资源",
        "api": "https://shayuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞豆豆资源",
        "api": "https://api.douapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞滴滴资源",
        "api": "https://api.ddapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黑料资源",
        "api": "https://www.heiliaozyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞百万资源",
        "api": "https://api.bwzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞桃花资源",
        "api": "https://thzy1.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞精品资源",
        "api": "https://www.jingpinx.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞 CK-资源",
        "api": "https://ckzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞souavZY",
        "api": "https://api.souavzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞细胞资源",
        "api": "https://www.xxibaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞香蕉资源",
        "api": "https://www.xiangjiaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-美少女-",
        "api": "https://www.msnii.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-黄AVZY",
        "api": "https://www.pgxdy.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞白嫖资源",
        "api": "https://www.kxgav.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞杏吧资源",
        "api": "https://xingba222.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞大地资源",
        "api": "https://dadiapi.com/feifei",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🔞色猫资源",
        "api": "https://caiji.semaozy.net/inc/apijson_vod.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞-奥斯卡-",
        "api": "https://aosikazy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞丝袜资源",
        "api": "https://siwazyw.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      }
    ]
  },
  {
    "date": "2026-06-28",
    "keyword": "你好",
    "results": [
      {
        "name": "🎬-爱奇艺-",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豆瓣资源",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🎬天涯资源",
        "api": "https://tyyszy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬天涯影视",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬茅台资源",
        "api": "https://caiji.maotaizy.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🎬卧龙资源",
        "api": "https://wolongzyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬iKun资源",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬电影天堂",
        "api": "http://caiji.dyttzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子资源",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬360 资源",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬极速资源",
        "api": "https://jszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🎬魔都资源",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬非凡资源",
        "api": "https://api.ffzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬暴风资源",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大资源",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽资源",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪资源",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬旺旺资源",
        "api": "https://api.wwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬速播资源",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰点播",
        "api": "https://www.jinyingzy.com/provide/vod?ct=xml",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬飘零资源",
        "api": "https://p2100.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷影视",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬光速资源",
        "api": "https://api.guangsuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🎬红牛资源",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都动漫",
        "api": "https://caiji.moduapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬如意资源",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华资源",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云zy",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬1080资源",
        "api": "https://api.1080zyku.com/inc/apijson.php/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无水印资源",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬艾旦影视",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://lovedan.net/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬量子影视",
        "api": "https://cj.lziapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大点播",
        "api": "https://zuidazy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽影视",
        "api": "https://api.wujinapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬旺旺短剧",
        "api": "https://wwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬优质资源",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬虎牙资源",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🎬鸭鸭资源",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬索尼资源",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬快车资源",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬闪电资源",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬樱花资源",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞麻豆视频",
        "api": "https://91md.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞--AIvin-",
        "api": "http://lbapiby.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞155-资源",
        "api": "https://155api.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞玉兔资源",
        "api": "https://apiyutu.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞番号资源",
        "api": "http://fhapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-老色逼-",
        "api": "https://apilsbzy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞优优资源",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://www.yytv4.cc/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞小鸡资源",
        "api": "https://api.xiaojizy.live/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黄色仓库",
        "api": "https://hsckzy.xyz/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞-大奶子-",
        "api": "https://apidanaizi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞jkun资源",
        "api": "https://jkunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞乐播资源",
        "api": "https://lbapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奶香资源",
        "api": "https://Naixxzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞森林资源",
        "api": "https://beiyong.slapibf.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞辣椒资源",
        "api": "https://apilj.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞鲨鱼资源",
        "api": "https://shayuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞豆豆资源",
        "api": "https://api.douapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞滴滴资源",
        "api": "https://api.ddapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黑料资源",
        "api": "https://www.heiliaozyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞百万资源",
        "api": "https://api.bwzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞桃花资源",
        "api": "https://thzy1.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞精品资源",
        "api": "https://www.jingpinx.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞 CK-资源",
        "api": "https://ckzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞souavZY",
        "api": "https://api.souavzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞细胞资源",
        "api": "https://www.xxibaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞香蕉资源",
        "api": "https://www.xiangjiaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-美少女-",
        "api": "https://www.msnii.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-黄AVZY",
        "api": "https://www.pgxdy.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞白嫖资源",
        "api": "https://www.kxgav.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞杏吧资源",
        "api": "https://xingba222.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞大地资源",
        "api": "https://dadiapi.com/feifei",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🔞色猫资源",
        "api": "https://caiji.semaozy.net/inc/apijson_vod.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞-奥斯卡-",
        "api": "https://aosikazy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞丝袜资源",
        "api": "https://siwazyw.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      }
    ]
  },
  {
    "date": "2026-06-29",
    "keyword": "你好",
    "results": [
      {
        "name": "🎬-爱奇艺-",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豆瓣资源",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬天涯资源",
        "api": "https://tyyszy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬天涯影视",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬茅台资源",
        "api": "https://caiji.maotaizy.cc/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬卧龙资源",
        "api": "https://wolongzyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬iKun资源",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬电影天堂",
        "api": "http://caiji.dyttzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子资源",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬360 资源",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬极速资源",
        "api": "https://jszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都资源",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬非凡资源",
        "api": "https://api.ffzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬暴风资源",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大资源",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽资源",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪资源",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬旺旺资源",
        "api": "https://api.wwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬速播资源",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰点播",
        "api": "https://www.jinyingzy.com/provide/vod?ct=xml",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬飘零资源",
        "api": "https://p2100.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷影视",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬光速资源",
        "api": "https://api.guangsuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛资源",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都动漫",
        "api": "https://caiji.moduapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬如意资源",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华资源",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云zy",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬1080资源",
        "api": "https://api.1080zyku.com/inc/apijson.php/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无水印资源",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬艾旦影视",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://lovedan.net/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬量子影视",
        "api": "https://cj.lziapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大点播",
        "api": "https://zuidazy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽影视",
        "api": "https://api.wujinapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬旺旺短剧",
        "api": "https://wwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬优质资源",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬虎牙资源",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬鸭鸭资源",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬索尼资源",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬快车资源",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬闪电资源",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬樱花资源",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞麻豆视频",
        "api": "https://91md.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞--AIvin-",
        "api": "http://lbapiby.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞155-资源",
        "api": "https://155api.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞玉兔资源",
        "api": "https://apiyutu.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞番号资源",
        "api": "http://fhapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-老色逼-",
        "api": "https://apilsbzy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞优优资源",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://www.yytv4.cc/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞小鸡资源",
        "api": "https://api.xiaojizy.live/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黄色仓库",
        "api": "https://hsckzy.xyz/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-大奶子-",
        "api": "https://apidanaizi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞jkun资源",
        "api": "https://jkunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞乐播资源",
        "api": "https://lbapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奶香资源",
        "api": "https://Naixxzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞森林资源",
        "api": "https://beiyong.slapibf.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞辣椒资源",
        "api": "https://apilj.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞鲨鱼资源",
        "api": "https://shayuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞豆豆资源",
        "api": "https://api.douapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞滴滴资源",
        "api": "https://api.ddapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黑料资源",
        "api": "https://www.heiliaozyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞百万资源",
        "api": "https://api.bwzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞桃花资源",
        "api": "https://thzy1.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞精品资源",
        "api": "https://www.jingpinx.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞 CK-资源",
        "api": "https://ckzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞souavZY",
        "api": "https://api.souavzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞细胞资源",
        "api": "https://www.xxibaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞香蕉资源",
        "api": "https://www.xiangjiaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-美少女-",
        "api": "https://www.msnii.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-黄AVZY",
        "api": "https://www.pgxdy.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞白嫖资源",
        "api": "https://www.kxgav.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞杏吧资源",
        "api": "https://xingba222.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞大地资源",
        "api": "https://dadiapi.com/feifei",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🔞色猫资源",
        "api": "https://caiji.semaozy.net/inc/apijson_vod.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞-奥斯卡-",
        "api": "https://aosikazy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞丝袜资源",
        "api": "https://siwazyw.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      }
    ]
  },
  {
    "date": "2026-06-30",
    "keyword": "你好",
    "results": [
      {
        "name": "🎬-爱奇艺-",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豆瓣资源",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬天涯资源",
        "api": "https://tyyszy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬天涯影视",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬茅台资源",
        "api": "https://caiji.maotaizy.cc/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬卧龙资源",
        "api": "https://wolongzyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬iKun资源",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬电影天堂",
        "api": "http://caiji.dyttzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子资源",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬360 资源",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬极速资源",
        "api": "https://jszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都资源",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬非凡资源",
        "api": "https://api.ffzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬暴风资源",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大资源",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽资源",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪资源",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬旺旺资源",
        "api": "https://api.wwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬速播资源",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰点播",
        "api": "https://www.jinyingzy.com/provide/vod?ct=xml",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬飘零资源",
        "api": "https://p2100.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷影视",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬光速资源",
        "api": "https://api.guangsuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛资源",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都动漫",
        "api": "https://caiji.moduapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬如意资源",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华资源",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云zy",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬1080资源",
        "api": "https://api.1080zyku.com/inc/apijson.php/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无水印资源",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬艾旦影视",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://lovedan.net/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬量子影视",
        "api": "https://cj.lziapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大点播",
        "api": "https://zuidazy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽影视",
        "api": "https://api.wujinapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬旺旺短剧",
        "api": "https://wwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬优质资源",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬虎牙资源",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬鸭鸭资源",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬索尼资源",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬快车资源",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬闪电资源",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬樱花资源",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞麻豆视频",
        "api": "https://91md.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞--AIvin-",
        "api": "http://lbapiby.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞155-资源",
        "api": "https://155api.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞玉兔资源",
        "api": "https://apiyutu.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞番号资源",
        "api": "http://fhapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-老色逼-",
        "api": "https://apilsbzy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞优优资源",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://www.yytv4.cc/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞小鸡资源",
        "api": "https://api.xiaojizy.live/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黄色仓库",
        "api": "https://hsckzy.xyz/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-大奶子-",
        "api": "https://apidanaizi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞jkun资源",
        "api": "https://jkunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞乐播资源",
        "api": "https://lbapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奶香资源",
        "api": "https://Naixxzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞森林资源",
        "api": "https://beiyong.slapibf.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞辣椒资源",
        "api": "https://apilj.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞鲨鱼资源",
        "api": "https://shayuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞豆豆资源",
        "api": "https://api.douapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞滴滴资源",
        "api": "https://api.ddapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黑料资源",
        "api": "https://www.heiliaozyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞百万资源",
        "api": "https://api.bwzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞桃花资源",
        "api": "https://thzy1.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞精品资源",
        "api": "https://www.jingpinx.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞 CK-资源",
        "api": "https://ckzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞souavZY",
        "api": "https://api.souavzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞细胞资源",
        "api": "https://www.xxibaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞香蕉资源",
        "api": "https://www.xiangjiaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-美少女-",
        "api": "https://www.msnii.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-黄AVZY",
        "api": "https://www.pgxdy.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞白嫖资源",
        "api": "https://www.kxgav.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞杏吧资源",
        "api": "https://xingba222.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞大地资源",
        "api": "https://dadiapi.com/feifei",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🔞色猫资源",
        "api": "https://caiji.semaozy.net/inc/apijson_vod.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞-奥斯卡-",
        "api": "https://aosikazy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞丝袜资源",
        "api": "https://siwazyw.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      }
    ]
  },
  {
    "date": "2026-07-01",
    "keyword": "你好",
    "results": [
      {
        "name": "🎬-爱奇艺-",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豆瓣资源",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🎬天涯资源",
        "api": "https://tyyszy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬天涯影视",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬茅台资源",
        "api": "https://caiji.maotaizy.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🎬卧龙资源",
        "api": "https://wolongzyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬iKun资源",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬电影天堂",
        "api": "http://caiji.dyttzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子资源",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬360 资源",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬极速资源",
        "api": "https://jszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都资源",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬非凡资源",
        "api": "https://api.ffzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬暴风资源",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大资源",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽资源",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪资源",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬旺旺资源",
        "api": "https://api.wwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬速播资源",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰点播",
        "api": "https://www.jinyingzy.com/provide/vod?ct=xml",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬飘零资源",
        "api": "https://p2100.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷影视",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬光速资源",
        "api": "https://api.guangsuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛资源",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都动漫",
        "api": "https://caiji.moduapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬如意资源",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华资源",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云zy",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬1080资源",
        "api": "https://api.1080zyku.com/inc/apijson.php/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无水印资源",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬艾旦影视",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://lovedan.net/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬量子影视",
        "api": "https://cj.lziapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大点播",
        "api": "https://zuidazy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽影视",
        "api": "https://api.wujinapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬旺旺短剧",
        "api": "https://wwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬优质资源",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬虎牙资源",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬鸭鸭资源",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬索尼资源",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬快车资源",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬闪电资源",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬樱花资源",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞麻豆视频",
        "api": "https://91md.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞--AIvin-",
        "api": "http://lbapiby.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞155-资源",
        "api": "https://155api.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞玉兔资源",
        "api": "https://apiyutu.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞番号资源",
        "api": "http://fhapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-老色逼-",
        "api": "https://apilsbzy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞优优资源",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://www.yytv4.cc/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞小鸡资源",
        "api": "https://api.xiaojizy.live/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黄色仓库",
        "api": "https://hsckzy.xyz/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞-大奶子-",
        "api": "https://apidanaizi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞jkun资源",
        "api": "https://jkunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞乐播资源",
        "api": "https://lbapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奶香资源",
        "api": "https://Naixxzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞森林资源",
        "api": "https://beiyong.slapibf.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞辣椒资源",
        "api": "https://apilj.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞鲨鱼资源",
        "api": "https://shayuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞豆豆资源",
        "api": "https://api.douapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞滴滴资源",
        "api": "https://api.ddapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黑料资源",
        "api": "https://www.heiliaozyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞百万资源",
        "api": "https://api.bwzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞桃花资源",
        "api": "https://thzy1.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞精品资源",
        "api": "https://www.jingpinx.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞 CK-资源",
        "api": "https://ckzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞souavZY",
        "api": "https://api.souavzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞细胞资源",
        "api": "https://www.xxibaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞香蕉资源",
        "api": "https://www.xiangjiaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-美少女-",
        "api": "https://www.msnii.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-黄AVZY",
        "api": "https://www.pgxdy.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞白嫖资源",
        "api": "https://www.kxgav.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞杏吧资源",
        "api": "https://xingba222.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞大地资源",
        "api": "https://dadiapi.com/feifei",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🔞色猫资源",
        "api": "https://caiji.semaozy.net/inc/apijson_vod.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞-奥斯卡-",
        "api": "https://aosikazy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞丝袜资源",
        "api": "https://siwazyw.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      }
    ]
  },
  {
    "date": "2026-07-02",
    "keyword": "你好",
    "results": [
      {
        "name": "🎬-爱奇艺-",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豆瓣资源",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🎬天涯资源",
        "api": "https://tyyszy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬天涯影视",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬茅台资源",
        "api": "https://caiji.maotaizy.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🎬卧龙资源",
        "api": "https://wolongzyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬iKun资源",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬电影天堂",
        "api": "http://caiji.dyttzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子资源",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬360 资源",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬极速资源",
        "api": "https://jszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬魔都资源",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬非凡资源",
        "api": "https://api.ffzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬暴风资源",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大资源",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽资源",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪资源",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬旺旺资源",
        "api": "https://api.wwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬速播资源",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰点播",
        "api": "https://www.jinyingzy.com/provide/vod?ct=xml",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬飘零资源",
        "api": "https://p2100.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷影视",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬光速资源",
        "api": "https://api.guangsuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛资源",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都动漫",
        "api": "https://caiji.moduapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬如意资源",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华资源",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云zy",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬1080资源",
        "api": "https://api.1080zyku.com/inc/apijson.php/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无水印资源",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬艾旦影视",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://lovedan.net/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬量子影视",
        "api": "https://cj.lziapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大点播",
        "api": "https://zuidazy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽影视",
        "api": "https://api.wujinapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬旺旺短剧",
        "api": "https://wwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬优质资源",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬虎牙资源",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬鸭鸭资源",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬索尼资源",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬快车资源",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬闪电资源",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬樱花资源",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞麻豆视频",
        "api": "https://91md.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞--AIvin-",
        "api": "http://lbapiby.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞155-资源",
        "api": "https://155api.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞玉兔资源",
        "api": "https://apiyutu.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞番号资源",
        "api": "http://fhapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-老色逼-",
        "api": "https://apilsbzy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞优优资源",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://www.yytv4.cc/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞小鸡资源",
        "api": "https://api.xiaojizy.live/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黄色仓库",
        "api": "https://hsckzy.xyz/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-大奶子-",
        "api": "https://apidanaizi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞jkun资源",
        "api": "https://jkunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞乐播资源",
        "api": "https://lbapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奶香资源",
        "api": "https://Naixxzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞森林资源",
        "api": "https://beiyong.slapibf.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞辣椒资源",
        "api": "https://apilj.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞鲨鱼资源",
        "api": "https://shayuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞豆豆资源",
        "api": "https://api.douapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞滴滴资源",
        "api": "https://api.ddapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黑料资源",
        "api": "https://www.heiliaozyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞百万资源",
        "api": "https://api.bwzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞桃花资源",
        "api": "https://thzy1.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞精品资源",
        "api": "https://www.jingpinx.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞 CK-资源",
        "api": "https://ckzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞souavZY",
        "api": "https://api.souavzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞细胞资源",
        "api": "https://www.xxibaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞香蕉资源",
        "api": "https://www.xiangjiaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-美少女-",
        "api": "https://www.msnii.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-黄AVZY",
        "api": "https://www.pgxdy.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞白嫖资源",
        "api": "https://www.kxgav.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞杏吧资源",
        "api": "https://xingba222.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞大地资源",
        "api": "https://dadiapi.com/feifei",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🔞色猫资源",
        "api": "https://caiji.semaozy.net/inc/apijson_vod.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞-奥斯卡-",
        "api": "https://aosikazy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞丝袜资源",
        "api": "https://siwazyw.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      }
    ]
  },
  {
    "date": "2026-07-03",
    "keyword": "你好",
    "results": [
      {
        "name": "🎬-爱奇艺-",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豆瓣资源",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🎬天涯资源",
        "api": "https://tyyszy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬天涯影视",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬茅台资源",
        "api": "https://caiji.maotaizy.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🎬卧龙资源",
        "api": "https://wolongzyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬iKun资源",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬电影天堂",
        "api": "http://caiji.dyttzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬猫眼资源",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子资源",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬360 资源",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬极速资源",
        "api": "https://jszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬魔都资源",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬非凡资源",
        "api": "https://api.ffzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬暴风资源",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大资源",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽资源",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪资源",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬旺旺资源",
        "api": "https://api.wwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬速播资源",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰点播",
        "api": "https://www.jinyingzy.com/provide/vod?ct=xml",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬飘零资源",
        "api": "https://p2100.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷影视",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬光速资源",
        "api": "https://api.guangsuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛资源",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都动漫",
        "api": "https://caiji.moduapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬如意资源",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华资源",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云zy",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬1080资源",
        "api": "https://api.1080zyku.com/inc/apijson.php/",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无水印资源",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬艾旦影视",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://lovedan.net/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬量子影视",
        "api": "https://cj.lziapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大点播",
        "api": "https://zuidazy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽影视",
        "api": "https://api.wujinapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬旺旺短剧",
        "api": "https://wwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬优质资源",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬虎牙资源",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬鸭鸭资源",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬索尼资源",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬快车资源",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬闪电资源",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬樱花资源",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞麻豆视频",
        "api": "https://91md.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞--AIvin-",
        "api": "http://lbapiby.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞155-资源",
        "api": "https://155api.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞玉兔资源",
        "api": "https://apiyutu.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞番号资源",
        "api": "http://fhapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-老色逼-",
        "api": "https://apilsbzy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞优优资源",
        "api": "https://lunacorsapi.passionariesapi.ccwu.cc/?url=https://www.yytv4.cc/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞小鸡资源",
        "api": "https://api.xiaojizy.live/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黄色仓库",
        "api": "https://hsckzy.xyz/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-大奶子-",
        "api": "https://apidanaizi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞jkun资源",
        "api": "https://jkunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞乐播资源",
        "api": "https://lbapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奶香资源",
        "api": "https://Naixxzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞森林资源",
        "api": "https://beiyong.slapibf.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞辣椒资源",
        "api": "https://apilj.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞鲨鱼资源",
        "api": "https://shayuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞豆豆资源",
        "api": "https://api.douapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞滴滴资源",
        "api": "https://api.ddapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黑料资源",
        "api": "https://www.heiliaozyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞百万资源",
        "api": "https://api.bwzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞桃花资源",
        "api": "https://thzy1.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞精品资源",
        "api": "https://www.jingpinx.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞 CK-资源",
        "api": "https://ckzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞souavZY",
        "api": "https://api.souavzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞细胞资源",
        "api": "https://www.xxibaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞香蕉资源",
        "api": "https://www.xiangjiaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-美少女-",
        "api": "https://www.msnii.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞-黄AVZY",
        "api": "https://www.pgxdy.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞白嫖资源",
        "api": "https://www.kxgav.com/api/json.php",
        "disabled": false,
        "success": true,
        "searchStatus": "✅"
      },
      {
        "name": "🔞杏吧资源",
        "api": "https://xingba222.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞大地资源",
        "api": "https://dadiapi.com/feifei",
        "disabled": false,
        "success": true,
        "searchStatus": "不匹配"
      },
      {
        "name": "🔞色猫资源",
        "api": "https://caiji.semaozy.net/inc/apijson_vod.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞-奥斯卡-",
        "api": "https://aosikazy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      },
      {
        "name": "🔞丝袜资源",
        "api": "https://siwazyw.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "searchStatus": "❌"
      }
    ]
  }
]
```
</details>
